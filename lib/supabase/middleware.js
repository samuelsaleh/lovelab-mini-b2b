 ...
 *
 * /api/me is the one route outside the portal every signed-in page needs: it
 * answers "who am I" with the account's own profile row and nothing else, and
 * the app renders nothing until it has. Refusing it to IGI (24 Sept 2026,
 * Michael's and Sam's first real IGI sign-in) left the portal with no profile,
 * so the layout sent them to /, which sent them back to /igi, and the page
 * stayed blank while the two bounced. /set-password is how they replace a
 * temporary password with their own.
 */
