...
// Holy has no dedicated necklace photography. All other necklace ids resolve
// their own manifest entries before any collection-specific fallback.
...
const HOUSING_ALIASES = {
  ...
  // Matchy two-metal codes (bezel and prong). The mixed ones (WY, WP, YP)
  // have no single-metal packshot, so they stay unaliased and fall through.
  WW: 'WG', YY: 'YG', PP: 'RG',
}
...
function normalizeHousing(raw) {
  if (!raw) return null
  // The builder stores Matchy / Shapy Shine housing as "Bezel WW" / "Prong Yellow";
  // drop the setting prefix so the alias table sees the bare code.
  const trimmed = raw.trim().replace(/^(Bezel|Prong)\s+/i, '')
 ...
 */
function findPackshotFromImages(images, opts = {}, { strictHousing = false } = {}) {
  ...
  if (h) {
    ...
    else if (strictHousing) return null
  }
...
function findShapyShineNecklacePackshot(opts) {
  const images = manifest.SSF_NECK || []
  const requestedHousing = normalizeHousing(opts.housing)
  const isCushion = String(opts.shape || '').toLowerCase() === 'cushion'

  // There is no dedicated Cushion necklace image. Sam approved the temporary
  // Long Cushion yellow-gold necklace image for Cushion, including white gold.
  if (isCushion) {
    const cushionFallback = findPackshotFromImages(images, {
      ...opts,
      shape: 'Long Cushion',
      housing: 'YG',
    }, { strictHousing: true })
    if (cushionFallback) return cushionFallback
  }

  const exact = findPackshotFromImages(images, opts, { strictHousing: true })
  if (exact) return exact

  // Some supplied Shapy Shine necklace variants only exist in yellow gold.
  // Reuse their yellow-gold necklace asset when white gold was selected.
  if (requestedHousing === 'WG') {
    const yellowGoldFallback = findPackshotFromImages(images, {
      ...opts,
      housing: 'YG',
    }, { strictHousing: true })
    if (yellowGoldFallback) return yellowGoldFallback
  }

  const looseNecklaceMatch = findPackshotFromImages(images, opts)
  if (looseNecklaceMatch) return looseNecklaceMatch

  // Preserve a visible thumbnail if a necklace variant has not been supplied.
  return findPackshotFromImages(manifest.SSF, opts)
}

function findShapySparkleNecklacePackshot(opts) {
  const images = manifest.SSPF_NECK || []
  const requestedShape = String(opts.shape || '').toLowerCase()
  const hasDedicatedShape = !requestedShape || images.some(
    (image) => image.shape?.toLowerCase() === requestedShape,
  )

  // Cushion and Emerald images have not been supplied yet, so retain the
  // bracelet image for precisely those missing shapes.
  if (!hasDedicatedShape) {
    return findPackshotFromImages(manifest.SSPF, opts)
  }

  const exact = findPackshotFromImages(images, opts, { strictHousing: true })
  if (exact) return exact

  // The supplied Shapy Sparkle necklace packshots are white-gold only. Keep a
  // relevant necklace image if another housing was selected.
  return findPackshotFromImages(images, opts) || findPackshotFromImages(manifest.SSPF, opts)
}

export function findPackshot(collectionId, opts = {}) {
  if (collectionId === 'SSF_NECK') {
    return findShapyShineNecklacePackshot(opts)
  }
  if (collectionId === 'SSPF_NECK') {
    return findShapySparkleNecklacePackshot(opts)
  }

  const images = manifest[resolveCollectionId(collectionId)]
  return findPackshotFromImages(images, opts)
}
...
const COLLECTION_LABELS = {
  ...
  M5_NECK: 'MULTI FIVE NECKLACE',
  SSF_NECK: 'SHAPY SHINE NECKLACE',
  CUBIX_NECK: 'CUBIX NECKLACE', MF_NECK: 'MATCHY FANCY NECKLACE',
  SSPF_NECK: 'SHAPY SPARKLE NECKLACE', HOLY_NECK: 'HOLY NECKLACE',
