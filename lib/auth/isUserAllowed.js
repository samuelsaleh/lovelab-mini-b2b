 ...
 * @returns {boolean}
 */
export function isUserAllowed({ isInAllowedEmails, agentProfile, isIgiEmail = false }) {
  if (isInAllowedEmails) return true;
  // Named in IGI_EMAILS: may sign in even before a profile exists (first login).
  if (isIgiEmail === true) return true;
