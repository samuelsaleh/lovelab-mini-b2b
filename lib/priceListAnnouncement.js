 ...
 */
import { renderEmail, BRAND, FONT_BODY } from './email-shell.js';
...
export function priceListAnnouncementEmail({
  ...
  const noteHtml = textToHtmlParagraphs(note, `${line}margin-bottom:12px;`);

  const collectionRows = (everyCollection ? [L.allCollections] : labels).map((name, i) => {
  ...
  const collectionsHtml = `
      <p style="${label}">${escapeHtml(L.collectionsLabel)}</p>
  ...
  // The one sentence agents must not miss: its own box, plum rule, no
  // competing emphasis anywhere else in the body.
  const inForceHtml = `
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:24px 0 8px;border-collapse:separate;">
        <tr><td style="padding:14px 16px;background:#f7f5f8;border-left:3px solid ${BRAND.plum};font-family:${FONT_BODY};font-size:15px;line-height:1.7;color:${BRAND.heading};">${escapeHtml(L.inForce)}</td></tr>
      </table>`;
  ...
  const bodyHtml = `
      ...
      <p style="${line}">${escapeHtml(L.intro)}</p>
      ${noteHtml ? `<p style="${label}">${escapeHtml(L.changesLabel)}</p>${noteHtml}` : ''}
      ${collectionsHtml}
      ${inForceHtml}
      ${fileHtml}
