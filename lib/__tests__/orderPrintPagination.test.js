...
describe('chunkRowsForPrint — argument hardening', () => {
  ...
  it('clamps lastPage to perPage when caller passes nonsense', () => {
    ...
    // lastPage (99) is clamped down to perPage (10). 15 rows then lay out as
    // a full page of 10 + a 5-row last page. (We're testing that nonsense
    // input is clamped and nothing crashes, not the exact geometry.)
    expect(out).toHaveLength(2)
    expect(out.map((p) => p.length)).toEqual([10, 5])
  })
})
