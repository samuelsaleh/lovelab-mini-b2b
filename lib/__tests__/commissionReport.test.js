...
describe('buildReportData — window filtering', () => {
  ...
  it('includes a March order that has customer_paid_at set (no period filter — all ready-to-pay included)', () => {
    // Report covers April but mom ticked a March order — it should appear.
    // The report shows everything ready to pay, regardless of order date.
    const data = buildReportData({
      agent: baseAgent,
      commissions: [mkOrder({
  ...
  it('includes an order from May even though period is April (all ready-to-pay included)', () => {
    const data = buildReportData({
      agent: baseAgent,
      commissions: [mkOrder({
  ...
  it('includes an order ticked in May even when it was placed in April', () => {
    const data = buildReportData({
      agent: baseAgent,
      commissions: [mkOrder({
        created_at: '2026-04-15T09:00:00Z',
  ...
  it('includes multiple orders with customer_paid_at set from any date', () => {
    const data = buildReportData({
      agent: baseAgent,
      commissions: [
        mkOrder({ customer_paid_at: '2026-05-07T10:00:00Z', commission_amount: 100, document: { client_company: 'March Order', created_at: '2026-03-01T00:00:00Z', order_channel: 'b2b' } }),
        mkOrder({ customer_paid_at: '2026-05-07T10:00:00Z', commission_amount: 200, document: { client_company: 'April Order', created_at: '2026-04-30T23:00:00Z', order_channel: 'b2b' } }),
      ],
      periodStart: PERIOD_START,
      periodEnd: PERIOD_END,
    });
