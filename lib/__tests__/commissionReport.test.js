...
describe('buildReportData — status filtering', () => {
  ...
  it('excludes commissions already linked to a report (Reported, awaiting payment)', () => {
    // Phase 29: once "Send report now" links a row to a report (report_id set),
    // it must not be swept into another report — it waits for Record Payment.
    const data = buildReportData({
      agent: baseAgent,
      commissions: [mkOrder({ status: 'pending', report_id: 'rep-1' })],
      periodStart: PERIOD_START,
      periodEnd: PERIOD_END,
    });
    expect(data.orders).toHaveLength(0);
    expect(data.includedCommissionIds).toHaveLength(0);
  });

  it('includes a pending row with no report_id (Ready)', () => {
    const data = buildReportData({
      agent: baseAgent,
      commissions: [mkOrder({ id: 'ready-1', status: 'pending', report_id: null })],
      periodStart: PERIOD_START,
      periodEnd: PERIOD_END,
    });
    expect(data.includedCommissionIds).toEqual(['ready-1']);
  });
