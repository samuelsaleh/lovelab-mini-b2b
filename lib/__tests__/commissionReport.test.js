...
describe('buildReportData — window filtering', () => {
  ...
  it('excludes commissions whose ORDER was created before the window starts', () => {
    // Order created in March — even if customer_paid_at is in April, it belongs to March's report
    const data = buildReportData({
      agent: baseAgent,
      commissions: [mkOrder({
        created_at: '2026-03-31T23:59:59Z',
        customer_paid_at: '2026-04-10T10:00:00Z', // paid in April, but order is March
        document: { id: 'doc-1', client_company: 'Acme Corp', total_amount: 1050, order_channel: 'b2b', created_at: '2026-03-31T23:59:59Z' },
      })],
      periodStart: PERIOD_START,
      periodEnd: PERIOD_END,
    });
  ...
  it('excludes commissions whose ORDER was created after the window ends', () => {
    // Order created in May — doesn't belong to April's report
    const data = buildReportData({
      agent: baseAgent,
      commissions: [mkOrder({
  ...
  it('includes an order from April even when customer_paid_at is in May (ticked late)', () => {
    // This is the real-world case: order from April, admin ticked it today
    const data = buildReportData({
      agent: baseAgent,
      commissions: [mkOrder({
        created_at: '2026-04-15T09:00:00Z',
        customer_paid_at: '2026-05-07T10:00:00Z', // ticked today
  ...
  it('includes both edges of the window (inclusive, by order date)', () => {
    const data = buildReportData({
      agent: baseAgent,
      commissions: [
        mkOrder({ customer_paid_at: '2026-05-07T10:00:00Z', commission_amount: 100, document: { client_company: 'Edge Start', created_at: '2026-04-01T00:00:00Z', order_channel: 'b2b' } }),
        mkOrder({ customer_paid_at: '2026-05-07T10:00:00Z', commission_amount: 200, document: { client_company: 'Edge End', created_at: '2026-04-30T23:00:00Z', order_channel: 'b2b' } }),
      ],
      periodStart: PERIOD_START,
      periodEnd: PERIOD_END,
    });
