...
describe('validateRow — N/A skipping', () => {
  ...
  it('requires Prong or Bezel for sparkleProngBezel but never a metal colour', () => {
    const dvvs = { ...sparkleProngCollection, housing: 'sparkleProngBezel' }
    expect(validateRow(completeRow({ setting: '', bpColor: '' }), dvvs).missing)
      .toEqual(expect.arrayContaining(['setting']))
    expect(validateRow(completeRow({ setting: 'Bezel', bpColor: '' }), dvvs).missing)
      .not.toContain('bpColor')
    expect(validateRow(completeRow({ setting: 'Bezel', bpColor: '' }), dvvs).missing)
...
describe('findCollection — defensive coercions', () => {
  ...
  it('still finds D VVS under the old catalogue names', () => {
    expect(findCollection('SHAPY SPARKLE RND D VVS')?.id).toBe('SSRD')
    expect(findCollection('SHAPY SPARKLE ROUND(D VVS)')?.id).toBe('SSRD')
    expect(findCollection('SHAPY SPARKLE D VVS')?.id).toBe('SSRD')
  })
})
...
describe('splitHousing — defensive coercions', () => {
  ...
  it('correctly parses Bezel/Prong prefixes', () => {
    ...
    expect(splitHousing('Bezel')).toEqual({ setting: 'Bezel', color: '' })
  })
...
// Collection that opts in to the bracelet thread closure column.
// Mirrors a real catalog entry like CUTY / CUBIX.
...
function closureRow(over = {}) {
  return {
    no: '1',
    quantity: '3',
    collection: 'TEST_CLOSURE',
    cert: 'IGI',
    carat: '0.10',
    shape: '',
    setting: '',
...
describe('validateRow — closure (CUTY/CUBIX) is optional everywhere', () => {
  // Sam's 2026-05-13 decision: closure is optional. Agents routinely
  // build large CUTY / CUBIX orders before the closure choice is locked
  // in with the client; flagging those rows as "missing closure" 80
  // times in a row drowned the genuinely-missing fields. Closure is now
  // never in the required list, regardless of `hasClosure`.

  it('passes when closure is set on a hasClosure collection', () => {
    const result = validateRow(closureRow(), closureCollection)
    expect(result.ok).toBe(true)
  })

  it('does NOT flag a missing closure on a hasClosure collection (optional)', () => {
    const result = validateRow(closureRow({ closure: '' }), closureCollection)
    expect(result.ok).toBe(true)
    expect(result.missing).not.toContain('closure')
  })

  it('does not require closure for non-hasClosure collections', () => {
    // Same row but the collection no longer opts in. Belt-and-braces: the
    // em-dash N/A path must keep working too.
    const noClosureCol = { ...closureCollection, hasClosure: false }
    const result = validateRow(closureRow({ closure: '' }), noClosureCol)
    expect(result.ok).toBe(true)
    expect(result.missing).not.toContain('closure')
  })

  it('does not list closure in the required fields for hasClosure collections', () => {
    const fields = getRequiredFieldsForRow(closureCollection)
    expect(fields).not.toContain('closure')
  })

  it('does not list closure in the required fields for plain collections', () => {
    const fields = getRequiredFieldsForRow(nylonCollection)
    expect(fields).not.toContain('closure')
  })

  it('aggregates correctly: 80 CUTY rows with empty closure validate clean', () => {
    // Direct repro of the screenshot Sam sent (80 CUTY rows producing
    // an 80-item "missing closure" banner). Post-fix, the order
    // validates without a single closure complaint as long as carat /
    // unitPrice / setting / etc. are filled.
    const rows = Array.from({ length: 80 }, (_, i) =>
      closureRow({ no: String(i + 1), closure: '' }),
    )
    const result = validateOrder(rows, () => closureCollection)
    expect(result.ok).toBe(true)
    expect(result.issues).toEqual([])
  })
})
