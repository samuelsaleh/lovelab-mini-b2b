...
describe('validateRow — closure (CUTY/CUBIX)', () => {
  it('passes when closure is set on a hasClosure collection', () => {
    const result = validateRow(closureRow(), closureCollection)
    expect(result.ok).toBe(true)
  })

  it('flags missing closure on a hasClosure collection', () => {
    const result = validateRow(closureRow({ closure: '' }), closureCollection)
    expect(result.ok).toBe(false)
    expect(result.missing).toContain('closure')
  })

  it('does not require closure for non-hasClosure collections', () => {
    // Same row but the collection no longer opts in. Even with empty closure
    // the row should validate, otherwise we'd retroactively block every old
    // order on save.
  ...
  it('lists closure in the required fields for hasClosure collections', () => {
    const fields = getRequiredFieldsForRow(closureCollection)
    expect(fields).toContain('closure')
  })
