...
describe('patchConsignmentOrder', () => {
  ...
  it('handles non-JSON error responses gracefully', async () => {
    ...
    await expect(
      patchConsignmentOrder('order-bad', {}, { returned_at: null })
    ).rejects.toThrow('Server error (502)')
  })

  it('returns parsed data on success', async () => {
    const doc = { id: 'order-3', metadata: { consignment: { returned_at: null } } }
    global.fetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ document: doc }),
    })

    const result = await patchConsignmentOrder('order-3', {}, { returned_at: null })
    expect(result.document.id).toBe('order-3')
  })
})
