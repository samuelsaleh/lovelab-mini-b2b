...
describe('SHAPY SPARKLE D VVS — cert by size', () => {
  ...
  it('cannot price 0.50 as IGI on 2026', () => {
    ...
    expect(getPrice(SSRD, 0, 'inhouse', '2026')).toBe(300)
  })
  ...
  it('inherits the same 2026 prices on the October list', () => {
    expect(getPrice(SSRD, 0, 'inhouse', '2026-10')).toBe(300)
    expect(getPrice(SSRD, 1, 'inhouse', '2026-10')).toBe(400)
    expect(getPrice(SSRD, 2, 'igi', '2026-10')).toBe(500)
