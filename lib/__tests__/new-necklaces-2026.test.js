...
// ─── Prices (B2B exact ×1.20, B2C rounded up to 5, identical both years) ──────
...
describe('new necklaces — prices', () => {
  ...
  it('2025 and 2026 are identical for every new necklace', () => {
    for (const c of ALL_NEW) {
      const cert = c.certificate
...
describe('new necklaces — packshots and labels', () => {
  it('reuse the source bracelet packshots', () => {
    expect(findPackshot('CUBIX_NECK', { color: 'Black' })).toEqual(findPackshot('CUBIX', { color: 'Black' }))
    expect(findPackshot('MF_NECK', { color: 'Black', shape: 'Heart' })).toEqual(findPackshot('MF', { color: 'Black', shape: 'Heart' }))
...
describe('new necklaces — calculateQuote', () => {
  ...
  it('prices a Shapy Sparkle necklace line (IGI, with shape) correctly', () => {
    ...
    expect(ln.unitB2B).toBe(288)
    expect(ln.retailUnit).toBe(660)
  })
})
