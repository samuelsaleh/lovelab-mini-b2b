...
describe('the default does not leak to collections that should still ask', () => {
  test('CUTY has no default closure', () => {
    const col = byId('CUTY')
    expect(getDefaultClosure(col)).toBeNull()
    expect(resolveClosure(col, null)).toBeNull()
  })

  test('CUBIX is pinned to non-braided, so a stored braided line normalises', () => {
    const col = byId('CUBIX')
    expect(getDefaultClosure(col)).toBe('nonBraided')
    expect(resolveClosure(col, 'braided')).toBe('nonBraided')
    expect(resolveClosure(col, null)).toBe('nonBraided')
    expect(closureOptionsFor(col)).toEqual(['nonBraided'])
    expect(sizeOptionsForClosure(col, 'braided')).toEqual(SIZES_SILK)
  })

  // Multi Three no longer asks: it is braided-only (Sam, 14 Sep 2026).
  test('M3 is pinned to braided, so a stored non-braided line normalises', () => {
    const col = byId('M3')
    expect(getDefaultClosure(col)).toBe('braided')
    expect(resolveClosure(col, 'nonBraided')).toBe('braided')
    expect(sizeOptionsForClosure(col, 'nonBraided')).toEqual(col.sizes)
  })
