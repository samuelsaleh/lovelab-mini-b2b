...
// The October 2026 list reprices Moonlight / Sienna / Za-Ha (admin/Piotr) and
// Iconix (every agent). Anyone who can sell a collection with a 2026-10 bucket
// is offered the list.
describe('canSeePricelist', () => {
  ...
  test('agents with Iconix preview can see it (Iconix is on the October list)', () => {
    for (const who of [BASTIAN, SARAH, OTHER_AGENT]) {
      expect(`${who.email}: ${canSeePricelist('2026-10', who)}`).toBe(`${who.email}: true`)
    }
  })
...
describe('getVisiblePricelists', () => {
  ...
  test('admins, Piotr and Iconix agents are offered all three lists', () => {
    expect(getVisiblePricelists(ADMIN)).toEqual(PRICELISTS)
    expect(getVisiblePricelists(PIOTR)).toEqual(PRICELISTS)
    expect(getVisiblePricelists(OTHER_AGENT)).toEqual(PRICELISTS)
  })

  test('unsigned surfaces are offered only the two public lists', () => {
  ...
  test('the active list is always offered, even when it is off-limits', () => {
    expect(getVisiblePricelists(null, '2026-10')).toEqual(PRICELISTS)
  })

  test('an off-limits list is dropped again once it is no longer active', () => {
    expect(getVisiblePricelists(null, '2026')).toEqual(['2025', '2026'])
  })

  test('never invents a list that is not in PRICELISTS', () => {
    expect(getVisiblePricelists(null, 'nonsense')).toEqual(['2025', '2026'])
