...
// Sam, 15 Sep 2026: only the October 2026 list is offered. Nothing is gated
// any more; the 2025 and pre-October 2026 lists are retired from the picker.
describe('canSeePricelist', () => {
  const ADMIN = { role: 'admin', email: 'admin@example.com' }

  test('no list is gated any more — October included', () => {
...
describe('getVisiblePricelists', () => {
  ...
  test('the 2025 and pre-October 2026 lists are retired from the picker', () => {
    expect([...RETIRED_PRICELISTS].sort()).toEqual(['2025', '2026'])
    expect(PRICELISTS.filter((y) => !RETIRED_PRICELISTS.has(y))).toEqual(['2026-10'])
  })

  test('everyone is offered the October list only, for a new order', () => {
    for (const who of [ADMIN, PIOTR, OTHER_AGENT, null]) {
      expect(getVisiblePricelists(who)).toEqual(['2026-10'])
    }
  })

  // An order saved on a retired list must keep showing which list priced it,
  // in catalog order, beside the current one — never silently relabelled.
  test('an order saved on a retired list keeps it beside October', () => {
    expect(getVisiblePricelists(OTHER_AGENT, '2025')).toEqual(['2025', '2026-10'])
    expect(getVisiblePricelists(ADMIN, '2026')).toEqual(['2026', '2026-10'])
  })

  test('never invents a list that is not in PRICELISTS', () => {
    expect(getVisiblePricelists(OTHER_AGENT, 'nonsense')).toEqual(['2026-10'])
    expect(getVisiblePricelists(ADMIN, 'nonsense')).toEqual(['2026-10'])
  })
})
