...
describe('collectionAccess — Bastian + Sarah Iconix preview', () => {
  test.each([BASTIAN, SARAH])('%s is granted exactly the 6 Iconix collection ids', (profile) => {
  ...
  test.each([BASTIAN, SARAH])('%s can see Iconix collections but not Moonlight/Sienna/Za-Ha', (profile) => {
  ...
  test.each([BASTIAN, SARAH])('getVisibleCollections(profile) includes legacy + Iconix for %s', (profile) => {
  ...
  test.each([BASTIAN, SARAH])('getPromptPreviewOptions returns partial preview for %s', (profile) => {
