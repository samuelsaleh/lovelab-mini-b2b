...
// October is the current default for everyone. 2025 / 2026 stay available
// only for Alberto and Dionne (email allowlist — not admin role).
describe('canSeePricelist', () => {
  ...
  const ALBERTO_GMAIL = { role: 'admin', email: 'albertosaleh@gmail.com' }

  test('legacy lists are gated — October is public', () => {
    expect([...LEGACY_PRICELISTS].sort()).toEqual(['2025', '2026'])
    expect([...PREVIEW_PRICELISTS]).toEqual([])
  })

  test('everyone can see the October list (including unsigned)', () => {
    for (const who of [ADMIN, PIOTR, OTHER_AGENT, BASTIAN, SARAH, ALBERTO, DIONNE, null, undefined]) {
      expect(canSeePricelist('2026-10', who)).toBe(true)
    }
  })

  test('Alberto and Dionne can see 2025 and 2026', () => {
    for (const who of [ALBERTO, ALBERTO_GMAIL, DIONNE]) {
      expect(canSeePricelist('2025', who)).toBe(true)
      expect(canSeePricelist('2026', who)).toBe(true)
    }
  })

  test('email match is case-insensitive and trims whitespace', () => {
    expect(canSeePricelist('2025', { email: '  Alberto@Love-Lab.com ' })).toBe(true)
    expect(canSeePricelist('2026', { email: 'DIONNE@LOVE-LAB.COM' })).toBe(true)
  })

  test('other agents and non-allowlisted admins cannot see 2025 or 2026', () => {
    for (const who of [ADMIN, PIOTR, OTHER_AGENT, BASTIAN, SARAH, null, undefined]) {
      expect(canSeePricelist('2025', who)).toBe(false)
      expect(canSeePricelist('2026', who)).toBe(false)
    }
  })

  test('accepts a numeric-ish year without crashing', () => {
    expect(canSeePricelist(2026, ALBERTO)).toBe(true)
    expect(canSeePricelist(2026, OTHER_AGENT)).toBe(false)
  })
})
...
describe('getVisiblePricelists', () => {
  ...
  test('Alberto and Dionne are offered all three lists', () => {
    expect(getVisiblePricelists(ALBERTO)).toEqual(PRICELISTS)
    expect(getVisiblePricelists(DIONNE)).toEqual(PRICELISTS)
  })

  test('other agents and non-allowlisted admins are offered only October', () => {
    expect(getVisiblePricelists(ADMIN)).toEqual(['2026-10'])
    expect(getVisiblePricelists(PIOTR)).toEqual(['2026-10'])
    expect(getVisiblePricelists(OTHER_AGENT)).toEqual(['2026-10'])
  })

  test('unsigned surfaces are offered only October', () => {
    expect(getVisiblePricelists(null)).toEqual(['2026-10'])
  })

  // A document priced on a legacy list can be reopened by anyone. Hiding the
  // list it was priced on would show old prices under an October label.
  test('the active list is always offered, even when it is off-limits', () => {
    expect(getVisiblePricelists(null, '2025')).toEqual(['2025', '2026-10'])
    expect(getVisiblePricelists(OTHER_AGENT, '2026')).toEqual(['2026', '2026-10'])
  })

  test('an off-limits list is dropped again once it is no longer active', () => {
    expect(getVisiblePricelists(null, '2026-10')).toEqual(['2026-10'])
  })

  test('never invents a list that is not in PRICELISTS', () => {
    expect(getVisiblePricelists(null, 'nonsense')).toEqual(['2026-10'])
    expect(getVisiblePricelists(ALBERTO, 'nonsense')).toEqual(PRICELISTS)
  })
})
