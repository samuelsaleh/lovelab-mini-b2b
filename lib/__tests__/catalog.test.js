...
describe('calculateQuote', () => {
  ...
  // Multi Three too (Sam, 14 Sep 2026: "het kan alleen maar in braided").
  it('pins M3 to "braided" whatever the config carries', () => {
    for (const stored of ['nonBraided', null, undefined]) {
      const result = calculateQuote([
        makeLine('M3', [makeConfig({ caratIdx: 0, qty: 1, closureType: stored })]),
      ])
      expect(result.lines).toHaveLength(1)
      expect(result.lines[0].closureType).toBe('braided')
    }
...
describe('COLLECTIONS — hasClosure flag', () => {
  ...
  // Multi Three joined them (Sam, 14 Sep 2026): the non-braided option is gone.
  it.each(['RIV4', 'RIV8', 'M3'])('%s opts in with a forced braided closure', (id) => {
    const col = COLLECTIONS.find(c => c.id === id)
    expect(col).toBeTruthy()
    expect(col.hasClosure).toBe(true)
...
describe('sizeOptionsForClosure', () => {
  ...
  // Multi Five SKUs only exist in the individual sizes in the Lovelab ERP, so a
  // non-braided line must still be XS–XL, never S/M or L/XL (Sam, Sep 2026).
  it('keeps MULTI FIVE at XS–XL whatever the closure (sizesIgnoreClosure)', () => {
  ...
  it('still groups MULTI FOUR (no opt-out) when non-braided', () => {
    const M4 = COLLECTIONS.find(c => c.id === 'M4')
    expect(M4.sizesIgnoreClosure).toBeUndefined()
    expect(sizeOptionsForClosure(M4, 'nonBraided')).toEqual(SIZES_SILK)
  })
