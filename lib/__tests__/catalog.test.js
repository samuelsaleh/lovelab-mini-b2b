...
describe('calculateQuote', () => {
  ...
  it('forwards closureType "nonBraided" for CUBIX (forced)', () => {
    const result = calculateQuote([
      makeLine('CUBIX', [makeConfig({ caratIdx: 0, qty: 1, closureType: 'braided' })]),
    ])
    expect(result.lines).toHaveLength(1)
    expect(result.lines[0].closureType).toBe('nonBraided')
  })

  it('pins CUBIX to "nonBraided" whatever the config carries', () => {
    for (const stored of ['braided', null, undefined]) {
      const result = calculateQuote([
        makeLine('CUBIX', [makeConfig({ caratIdx: 0, qty: 1, closureType: stored })]),
      ])
      expect(result.lines).toHaveLength(1)
      expect(result.lines[0].closureType).toBe('nonBraided')
    }
  ...
  it('forwards closureType for the newly-enabled nylon bracelets (MF/HOLY)', () => {
    for (const id of ['MF', 'HOLY']) {
      const result = calculateQuote([
  ...
  // Shapy Shine is braided-only (Alberto, Aug 2026), so a line saved back when
  // non-braided was still on offer must come back as braided rather than
  // quoting a closure the collection no longer sells.
  it('pins SSF to "braided" whatever the config carries', () => {
    for (const stored of ['nonBraided', null, undefined]) {
      const result = calculateQuote([
        makeLine('SSF', [makeConfig({ caratIdx: 0, qty: 1, closureType: stored })]),
      ])
      expect(result.lines).toHaveLength(1)
      expect(result.lines[0].closureType).toBe('braided')
    }
  ...
  // Multi Three / Four / Five too — braided-only.
  it.each(['M3', 'M4', 'M5'])('pins %s to "braided" whatever the config carries', (id) => {
    for (const stored of ['nonBraided', null, undefined]) {
      const result = calculateQuote([
        makeLine(id, [makeConfig({ caratIdx: 0, qty: 1, closureType: stored })]),
      ])
      expect(result.lines).toHaveLength(1)
      expect(result.lines[0].closureType).toBe('braided')
    }
...
describe('COLLECTIONS — hasClosure flag', () => {
  ...
  it('Shapy Sparkle and Sienna silk bracelets do NOT opt in to the closure column', () => {
    for (const id of ['SSPF', 'SSRG', 'SSRD', 'SI1']) {
      const col = COLLECTIONS.find(c => c.id === id)
      expect(col).toBeTruthy()
  ...
  // Iconix silk bracelets DO opt in (Sam, Sep 2026): braided ships in S / M / L,
  // non-braided in the grouped silk sizes. Za-Ha and the Flower pieces default
  // to non-braided so the orders saved before the closure existed — all in S/M
  // or L/XL — stay valid.
  it.each(['ZAHA', 'LUVA', 'LUMA'])('%s (Iconix silk) opts in with a non-braided default', (id) => {
    const col = COLLECTIONS.find(c => c.id === id)
    expect(col).toBeTruthy()
    expect(col.hasClosure).toBe(true)
    expect(col.defaultClosure).toBe('nonBraided')
    expect(col.forcedClosure).toBeUndefined()
  })

  // Riviera / Multi Three–Five are braided-only. CUBIX is non-braided-only.
  it.each(['RIV4', 'RIV8', 'M3', 'M4', 'M5'])('%s opts in with a forced braided closure', (id) => {
    const col = COLLECTIONS.find(c => c.id === id)
    expect(col).toBeTruthy()
    expect(col.hasClosure).toBe(true)
    expect(col.forcedClosure).toBe('braided')
    expect(col.defaultClosure).toBeUndefined()
  })

  it('CUBIX opts in with a forced non-braided closure', () => {
    const col = COLLECTIONS.find(c => c.id === 'CUBIX')
    expect(col).toBeTruthy()
    expect(col.hasClosure).toBe(true)
    expect(col.forcedClosure).toBe('nonBraided')
    expect(col.defaultClosure).toBeUndefined()
  })
...
describe('sizeOptionsForClosure', () => {
  ...
  it('returns grouped silk sizes for CUBIX (forced non-braided)', () => {
    expect(CUBIX.forcedClosure).toBe('nonBraided')
    expect(sizeOptionsForClosure(CUBIX, 'braided')).toEqual(SIZES_SILK)
  ...
  // Multi Five SKUs only exist in the individual sizes in the Lovelab ERP.
  // Forced braided + sizesIgnoreClosure: always XS–XL (Sam, Sep 2026).
  it('keeps MULTI FIVE at XS–XL (forced braided + sizesIgnoreClosure)', () => {
    const M5 = COLLECTIONS.find(c => c.id === 'M5')
    expect(M5.forcedClosure).toBe('braided')
    expect(M5.sizesIgnoreClosure).toBe(true)
    expect(M5.hasClosure).toBe(true)
    for (const closure of ['braided', 'nonBraided', null, undefined]) {
      expect(sizeOptionsForClosure(M5, closure)).toEqual(['XS', 'S', 'M', 'L', 'XL'])
      expect(sizeOptionsForClosure(M5, closure)).not.toContain('S/M')
      expect(sizeOptionsForClosure(M5, closure)).not.toContain('L/XL')
    }
  })

  // Forced braided wins over a stored nonBraided value, so sizes stay nylon XS–XL.
  it('keeps MULTI FOUR at XS–XL even when a non-braided value is passed (forced)', () => {
    const M4 = COLLECTIONS.find(c => c.id === 'M4')
    expect(M4.forcedClosure).toBe('braided')
    expect(M4.sizesIgnoreClosure).toBeUndefined()
    expect(sizeOptionsForClosure(M4, 'nonBraided')).toEqual(['XS', 'S', 'M', 'L', 'XL'])
    expect(sizeOptionsForClosure(M4, 'braided')).toEqual(['XS', 'S', 'M', 'L', 'XL'])
  })
