...
describe('sizeOptionsForClosure', () => {
  ...
  it('is a no-op for collections without hasClosure (M3) regardless of closure', () => {
    expect(sizeOptionsForClosure(M3, 'nonBraided')).toEqual(M3.sizes)
    expect(sizeOptionsForClosure(M3, 'braided')).toEqual(M3.sizes)
    expect(sizeOptionsForClosure(M3, null)).toEqual(M3.sizes)
  })
