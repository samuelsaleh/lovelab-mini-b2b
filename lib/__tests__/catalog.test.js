...
describe('COLLECTIONS — hasClosure flag', () => {
  ...
  // non-braided in the grouped silk sizes. They default to non-braided so the
  // orders saved before the closure existed — all in S/M or L/XL — stay valid.
  it.each(['ZAHA', 'LUVA', 'LUMA', 'RIV4', 'RIV8'])('%s (Iconix silk) opts in with a non-braided default', (id) => {
    const col = COLLECTIONS.find(c => c.id === id)
    expect(col).toBeTruthy()
    expect(col.hasClosure).toBe(true)
