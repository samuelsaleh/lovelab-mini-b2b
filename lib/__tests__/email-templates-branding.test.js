...
const {
  ...
  approvedSignupEmail,
  orgInvitationEmail,
  welcomeEmployeeWithPasswordEmail,
  upgradeEmployeeEmail,
...
describe('lib/email-templates.js — LoveLab branding', () => {
  ...
  it('welcomeEmployeeWithPasswordEmail avoids "LoveLab B2B" and includes the credentials', () => {
    const { subject, html } = welcomeEmployeeWithPasswordEmail('Christelle', 'c@test.com', 'Christelle4821!', `${SITE_URL}/login`, SITE_URL)
    expect(subject).not.toMatch(/LoveLab B2B/)
    expect(html).not.toMatch(/LoveLab B2B/)
    expect(subject).toMatch(/LoveLab/)
    expect(html).toContain('c@test.com')
    expect(html).toContain('Christelle4821!')
    expect(html).toContain(`${SITE_URL}/login`)
  })

  it('upgradeEmployeeEmail avoids "LoveLab B2B"', () => {
    const { subject, html } = upgradeEmployeeEmail('Christelle', SITE_URL)
    expect(subject).not.toMatch(/LoveLab B2B/)
    expect(html).not.toMatch(/LoveLab B2B/)
    expect(subject).toMatch(/full access/)
  })
