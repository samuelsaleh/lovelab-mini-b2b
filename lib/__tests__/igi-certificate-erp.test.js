...
import { syncCertificateIns, isB2bOriginatedIn } from '../igi/syncCertificateIns'
...
function makeSyncDb(table, { lastId = null, models = [], idKey = 'erp_out_id' } = {}) {
        ...
        return {
          select: () => ({
            order: () => ({
              limit: () => ({
                maybeSingle: async () => ({
                  data: lastId == null ? null : { [idKey]: lastId },
                  error: null,
                }),
              }),
            }),
...
describe('syncCertificateIns', () => {
  it('upserts ERP certificate-in rows and matches by serial', async () => {
    ...
    expect(writes.upserts[0].erp_in_id).toBe(5)
  })
})
