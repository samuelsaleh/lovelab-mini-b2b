...
describe('priceListAnnouncementEmail', () => {
  ...
  test('a non-English email carries the English version below its own, English emails do not repeat it', () => {
    const de = priceListAnnouncementEmail({ ...base, lang: 'de', note: 'CUTY +5 %.', englishNote: 'CUTY up 5%.' }, SITE).html
    expect(de).toContain('English version')
    expect(de).toContain('CUTY +5 %.')
    expect(de).toContain('CUTY up 5%.')
    expect(de.indexOf('Was sich geändert hat')).toBeLessThan(de.indexOf('What has changed'))
    expect(de.indexOf('What has changed')).toBeLessThan(de.indexOf('Mit freundlichen Grüßen'))
    expect(de).toContain('this is the price list in force')

    const en = priceListAnnouncementEmail({ ...base, lang: 'en', note: 'CUTY up 5%.', englishNote: 'CUTY up 5%.' }, SITE).html
    expect(en).not.toContain('English version')
    expect(en.split('CUTY up 5%.').length - 1).toBe(1)

    const noEnglish = priceListAnnouncementEmail({ ...base, lang: 'fr', note: 'CUTY +5 %.' }, SITE).html
    expect(noEnglish).not.toContain('English version')
  })
