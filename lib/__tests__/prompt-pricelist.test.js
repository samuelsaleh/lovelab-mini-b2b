...
describe('buildPricesBlock', () => {
  ...
  test('invalid year falls back to default (2026-10)', () => {
    expect(buildPricesBlock('2024')).toBe(buildPricesBlock('2026-10'));
    expect(buildPricesBlock(undefined)).toBe(buildPricesBlock('2026-10'));
  });

  // The October list reprices Moonlight / Sienna / Iconix bracelets, so the AI
  // must quote the new numbers there while leaving every classic collection alone.
  describe('October 2026 list', () => {
    ...
    test('October-only sizes never leak into the 2026 block', () => {
      ...
      expect(oct).toContain('0.70=€200/€600');
      expect(oct).toContain('1.01=€320/€960');
      expect(y26).not.toContain('0.70=');
      expect(y26).not.toContain('1.01=€320');
    });
...
describe('buildSystemPrompt', () => {
  ...
  test('legacy SYSTEM_PROMPT constant equals buildSystemPrompt(default)', () => {
    expect(SYSTEM_PROMPT).toBe(buildSystemPrompt('2026-10'));
  });
});
