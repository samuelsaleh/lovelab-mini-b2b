...
describe('order email catalogue selection', () => {
  ...
  test('returns null when neither the requested catalogue nor English fallback exists', async () => {
    await expect(readOrderEmailCatalogue('de', { publicDirectory: '/definitely/missing' }))
      .resolves.toBeNull();
  });
});
