...
const cataloguePath = (rel) => path.join(process.cwd(), 'public', 'catalogues', rel);
...
describe('order email catalogue selection', () => {
  ...
  test('every language resolves to a catalogue that exists on disk', () => {
    for (const candidates of Object.values(ORDER_EMAIL_CATALOGUE_CANDIDATES)) {
      for (const rel of candidates) {
        expect(fs.existsSync(cataloguePath(rel))).toBe(true);
      }
    }
  });

  // lib/orderEmailCatalogue.js reads its filename dynamically, so @vercel/nft
  // bundles the WHOLE catalogues directory into api/documents/send-email.
  // Keeping a print master beside each email copy put that function at 256 MB
  // and failed the deploy on Vercel's 250 MB limit. One copy per catalogue,
  // already small, is the rule — this test is what enforces it.
  test('the catalogues directory stays small enough to bundle into the function', () => {
    const root = path.join(process.cwd(), 'public', 'catalogues');
    const walk = (dir) => fs.readdirSync(dir, { withFileTypes: true }).flatMap((e) => {
      const full = path.join(dir, e.name);
      return e.isDirectory() ? walk(full) : [fs.statSync(full).size];
    });
    const totalMb = walk(root).reduce((a, b) => a + b, 0) / 1024 / 1024;
    expect(totalMb).toBeLessThan(150);
  });

  test('no catalogue is duplicated under an email/ subfolder', () => {
    expect(fs.existsSync(path.join(process.cwd(), 'public', 'catalogues', 'email'))).toBe(false);
  });

  test('every catalogue on disk is within the attachment budget', () => {
    const root = path.join(process.cwd(), 'public', 'catalogues');
    const walk = (dir) => fs.readdirSync(dir, { withFileTypes: true }).flatMap((e) => {
      const full = path.join(dir, e.name);
      if (e.isDirectory()) return walk(full);
      return e.name.toLowerCase().endsWith('.pdf') ? [[full, fs.statSync(full).size]] : [];
    });
    for (const [file, size] of walk(root)) {
      expect({ file, ok: size <= MAX_CATALOGUE_BYTES }).toEqual({ file, ok: true });
    }
  });
  ...
  test.each(Object.entries(ORDER_EMAIL_CATALOGUE_CANDIDATES))(
    ...
    (_lang, candidates) => {
      for (const rel of candidates) {
        const { size } = fs.statSync(cataloguePath(rel));
  ...
  test('serves the whole file it found on disk', async () => {
    ...
    expect(result.buffer.length).toBe(fs.statSync(cataloguePath(rel)).size);
  });
