...
describe('necklace packshots', () => {
  test.each([
    ...
    ['SSPF_NECK', { color: 'Black', housing: 'White', shape: 'Round', subgroup: 'Prong' }],
  ...
  it('indexes every dedicated necklace collection in the manifest', () => {
    for (const id of ['CUTY_NECK', 'CUBIX_NECK', 'M3_NECK', 'M4_NECK', 'M5_NECK', 'MF_NECK', 'SSF_NECK', 'SSPF_NECK']) {
  ...
  it('uses the supplied white-gold Matchy Emerald necklace images', () => {
    const url = findPackshot('MF_NECK', {
      color: 'Black',
      housing: 'White',
      shape: 'Emerald',
      subgroup: 'Bezel',
    });
    expect(url).toContain('/Necklaces/matchy_necks/emerald%20matchy/')
    expect(url).toContain('Black_white_gold_0_3ct_nylon-0ibkov_necklace.png')
    expect(fs.existsSync(publicPath(url))).toBe(true);
  });

  it('keeps bracelet fallback images only for Shapy Sparkle shapes without supplied assets', () => {
    expect(findPackshot('SSPF_NECK', { color: 'Black', shape: 'Cushion' }))
      .toEqual(findPackshot('SSPF', { color: 'Black', shape: 'Cushion' }));
    expect(findPackshot('SSPF_NECK', { color: 'Black', shape: 'Emerald' }))
      .toEqual(findPackshot('SSPF', { color: 'Black', shape: 'Emerald' }));
