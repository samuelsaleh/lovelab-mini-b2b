...
describe('generateAgentReport', () => {
  ...
  test('happy path: builds, uploads, drives, emails, inserts row', async () => {
    ...
    const res = await generateAgentReport({
      ...
      agentId: 'agent-1',
      period: PERIOD,
      options: { triggerSource: 'manual', triggeredBy: 'admin-1', sendEmail: true },
    });
    ...
    // Phase 29: generation links the rows to the report (Reported state) but
    // does NOT mark them paid — that happens at Record Payment time.
    expect(res.linked).toEqual({ linked: 2, ids: ['comm-1', 'comm-2'] });

    const commUpdates = supabase.from.mock.results
      .map((r) => r.value)
      .filter((v) => v?._updatePayload && 'report_id' in v._updatePayload);
    expect(commUpdates.length).toBeGreaterThan(0);
    expect(commUpdates[0]._updatePayload.report_id).toBe('r-inserted');
    expect(commUpdates[0]._updateInIds).toEqual(['comm-1', 'comm-2']);
    // No update should set status:'paid' during generation anymore.
    const paidUpdates = supabase.from.mock.results
      .map((r) => r.value)
      .filter((v) => v?._updatePayload?.status === 'paid');
    expect(paidUpdates.length).toBe(0);
  });
  ...
  test('does not email by default (sendEmail=false)', async () => {
    const { generateAgentReport } = require('../commissionReportService.js');
