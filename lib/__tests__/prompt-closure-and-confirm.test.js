...
describe('AI advisor prompt — closure + double-check rules', () => {
  ...
  test('lists closure as a required field for CUTY (CUBIX is forced non-braided)', () => {
    expect(prompt).toMatch(/Closure.*CUTY/i)
    expect(prompt).toMatch(/CUBIX is non-braided-only/i)
