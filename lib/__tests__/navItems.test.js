...
describe('ADMIN_NAV_ITEMS (reorganized with submenus, Aug 2026)', () => {
  const flatIds = flattenNavItems(ADMIN_NAV_ITEMS).map((i) => i.id)

  it('top level is decluttered: Dashboard, Sales Team, Out Memos, Internal Orders, Fairs, Certificates, Analytics, Customer Reports, Back', () => {
    expect(ADMIN_NAV_ITEMS.map((i) => i.id)).toEqual([
      'dashboard',
      'sales-team',
      'out-memos',
      'internal-orders',
  ...
  it('Out Memos is its own menu item after Sales Team', () => {
    const outMemos = ADMIN_NAV_ITEMS.find((i) => i.id === 'out-memos')
    expect(outMemos.href).toBe('/admin/out-memos')
    expect(outMemos.label).toBe('Out Memos')
  })

  it('Internal Orders lives in admin, not the main app sidebar', () => {
    const item = ADMIN_NAV_ITEMS.find((i) => i.id === 'internal-orders')
    expect(item.href).toBe('/admin/internal-orders')
    expect(item.label).toBe('Internal Orders')
  })

  it('Sales Team is one menu item; roles are tabs inside the workspace', () => {
    const salesTeam = ADMIN_NAV_ITEMS.find((i) => i.id === 'sales-team')
    expect(salesTeam.href).toBe('/admin/agents')
    expect(salesTeam.children).toBeUndefined()
    expect(flatIds).not.toContain('agents')
    expect(flatIds).not.toContain('assistants')
    expect(flatIds).not.toContain('organizations')
  })

  it('Fairs group merges the fair pages (All Fairs + Fair Assistant)', () => {
    const fairs = ADMIN_NAV_ITEMS.find((i) => i.id === 'fairs-group')
    expect(fairs.children.map((c) => c.id)).toEqual(['fairs', 'fair-assistant'])
  })

  it('Consignment is gone from the menu (page still exists, just unlisted)', () => {
    expect(flatIds).not.toContain('consignment')
  })

  it('Reports is renamed Customer Reports and still routes to /admin/reports', () => {
    const reports = ADMIN_NAV_ITEMS.find((i) => i.id === 'reports')
    expect(reports.label).toBe('Customer Reports')
    expect(reports.href).toBe('/admin/reports')
  })

  it('every leaf item has an href (groups have children instead)', () => {
    for (const item of flattenNavItems(ADMIN_NAV_ITEMS)) {
      expect(item.href).toBeTruthy()
      expect(item.children).toBeUndefined()
    }
  })

  it('flattenNavItems keeps flat lists untouched', () => {
    expect(flattenNavItems(AGENT_NAV_ITEMS)).toEqual(AGENT_NAV_ITEMS)
  })
})
...
describe('getMainNavItems', () => {
  it('returns role-appropriate items for an agent', () => {
    ...
    expect(items.map((i) => i.label)).not.toContain('AI Advisor')
  })
  ...
  it('returns admin-only items for admins', () => {
    ...
    expect(items.map((i) => i.label)).not.toContain('AI Advisor')
    expect(ids).not.toContain('internal_orders')
    expect(ids).not.toContain('internal-orders')
  })
  ...
  it('gives commercial assistants a focused nav: Home, Builder, Documents only', () => {
    const ids = getMainNavItems({ role: 'member', is_assistant: true }).map((i) => i.id)
    expect(ids).toEqual(['home', 'builder', 'documents'])
  })

  it('an admin who is also flagged assistant keeps the full admin nav', () => {
    const ids = getMainNavItems({ role: 'admin', is_assistant: true }).map((i) => i.id)
    expect(ids).not.toContain('ai')
    expect(ids).toContain('photos')
  })

  it('does not duplicate Assistants as a separate admin menu item', () => {
    const ids = flattenNavItems(ADMIN_NAV_ITEMS).map((i) => i.id)
