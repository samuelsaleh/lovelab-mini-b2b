...
describe('listPackTemplates', () => {
  it('maps global+seed packs to {id, label, fileName}', async () => {
    const admin = makeAdmin({
      fromResult: {
        data: [
          { id: 'p1', label: 'Pack 1', scope: 'global', is_seed: true },
          { id: 'p2', label: 'PACK 6-RB-SYN', scope: 'global', is_seed: true },
        ],
        error: null,
      },
    })
    ...
    expect(out).toEqual([
      ...
      { id: 'p2', label: 'PACK 6-RB-SYN', fileName: 'LoveLab_Order_Template_PACK_6-RB-SYN.xlsx' },
    ])
  })
})
