import { ALLOWED_RESOURCE_PATH_RE, publicAssetHref, publicAssetUrl } from '../publicAssetHref';
...
describe('publicAssetHref', () => {
  ...
  test('encodes Price Lists and Ean Codes folders', () => {
    ...
    expect(publicAssetHref('/Price Lists/Pricelist_LoveLab_2026_October.pdf')).toBe(
      '/Price%20Lists/Pricelist_LoveLab_2026_October.pdf',
    );
    expect(publicAssetHref('/Ean Codes/Final-GS1-Code.xlsx')).toBe(
      '/Ean%20Codes/Final-GS1-Code.xlsx',
    );
    expect(publicAssetHref('/IGI Excel/IGI_ORDERS_FILL.xlsx')).toBe(
      '/IGI%20Excel/IGI_ORDERS_FILL.xlsx',
    );
  });

  test('builds an encoded absolute URL for Fair Assistant attachments', () => {
    expect(publicAssetUrl(
      'https://b2b-lovelab.com/',
      '/catalogues/Oct PL_LoveLab_B2B_Catalogue General (210 x 210 mm).pdf',
    )).toBe(
      'https://b2b-lovelab.com/catalogues/Oct%20PL_LoveLab_B2B_Catalogue%20General%20(210%20x%20210%20mm).pdf',
    );
  });

  test('Fair Assistant uses the encoded URL helper for static attachments', () => {
    const route = fs.readFileSync(
      path.join(process.cwd(), 'app/api/fair-assistant/send/route.js'),
      'utf8',
    );
    expect(route).toContain('publicAssetUrl(siteOrigin, path)');
    expect(route).not.toContain("`${siteOrigin.replace(/\\/$/, '')}${path}`");
  });
});
...
describe('ALLOWED_RESOURCE_PATH_RE', () => {
  test('accepts nested catalogue paths used on the home page', () => {
    ...
    expect(ALLOWED_RESOURCE_PATH_RE.test(
      '/catalogues/English/Oct EN_LoveLab_B2B_Catalogue (210 x 210 mm).pdf',
    )).toBe(true);
  });
  ...
  test('accepts price lists and EAN codes', () => {
    ...
    expect(ALLOWED_RESOURCE_PATH_RE.test('/Price Lists/Pricelist_LoveLab_2026_October.pdf')).toBe(true);
    expect(ALLOWED_RESOURCE_PATH_RE.test('/Ean Codes/Final-GS1-Code.xlsx')).toBe(true);
  });

  test('accepts the IGI Excel used on Home Documents', () => {
    expect(ALLOWED_RESOURCE_PATH_RE.test('/IGI Excel/IGI_ORDERS_FILL.xlsx')).toBe(true);
  });
