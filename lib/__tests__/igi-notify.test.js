 ...
 */
import { notifyIgiOfRequest, notifyLovelabOfIssue, notifyIgiOfShortReturn, notifyIgiOfNewModel, igiRecipients, describeFailure } from '../igi/notify'
...
describe('notifyIgiOfNewModel (Sam, 24 Sept 2026)', () => {
  const waiting = { id: 'm9', name: 'Full Moonlight', stones: '1', carat: 0.5, shape: 'Round', state: 'awaiting_serial' }
  it('tells IGI to number the model', async () => {
    const sendEmail = jest.fn(async () => ({ sent: true }))
    const r = await notifyIgiOfNewModel(db(tables({ igi_models: [...MODELS, waiting] })), { modelId: 'm9', siteUrl: 'https://app.test' }, { sendEmail, env: { IGI_EMAILS: '' } })
    expect(r).toEqual({ sent: true, recipients: ['michael@igi.org'] })
    const mail = sendEmail.mock.calls[0][0]
    expect(mail.subject).toBe('New certificate model from LoveLab: please give it an IGI serial')
    expect(mail.html).toContain('Full Moonlight')
    expect(mail.html).toContain('https://app.test/igi')
  })
  it('records a health event when the mail fails, and never throws', async () => {
    const recordHealthEvent = jest.fn(async () => ({ ok: true }))
    const r = await notifyIgiOfNewModel(db(tables({ igi_models: [waiting] })), { modelId: 'm9' }, { sendEmail: async () => ({ sent: false, reason: 'network_error' }), recordHealthEvent, env: { IGI_EMAILS: '' } })
    expect(r.sent).toBe(false)
    expect(recordHealthEvent).toHaveBeenCalledWith(expect.objectContaining({ source: 'igi_new_model_email', severity: 'warn' }))
  })
})
