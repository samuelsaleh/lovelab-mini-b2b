...
describe('2026 pricelist matches public/Price Lists/Pricelist_LoveLab_2026.pdf', () => {
  ...
  describe('SHAPY SPARKLE RND G/H (inhouse)', () => {
    const c = col('SSRG');
    test.each([
      [0, 'inhouse', 160, 350],  // 0,50
      [1, 'inhouse', 200, 450],  // 0,70
      [2, 'inhouse', 260, 700],  // 1,00
  ...
  describe('SHAPY SPARKLE D VVS (inhouse 0.50/0.70, igi 1.00)', () => {
    const c = col('SSRD');
    test.each([
      [0, 'inhouse', 300, 800],   // 0,50
      [1, 'inhouse', 400, 1000],  // 0,70
      [2, 'igi',     500, 1200],  // 1,00
  ...
  // B2C is asserted here too since 2026-08-13: the app had kept the original
  // workbook's retail column, which sat below what this PDF shows clients.
  describe('ICONIX (PDF 2026-07-19)', () => {
    test.each([
      // [id,     caratIdx, B2B, B2C]
      ['LUVA',    0,        150, 585],  // Flower Heart 0,40
      ['LUMA',    0,        130, 495],  // Flower Marquise 0,40
      ['LIN3',    0,        115, 430],  // Linea Three 0,30
      ['LIN5',    0,        175, 675],  // Linea Five 0,50
      ['RIV4',    0,        90,  340],  // Riviera Four 0,20
      ['RIV4',    1,        115, 430],  // Riviera Four 0,40
      ['RIV8',    0,        115, 430],  // Riviera Eight 0,40
      ['RIV8',    1,        150, 585],  // Riviera Eight 0,80
    ])('%s caratIdx=%i -> B2B=%i B2C=%i', (id, ci, b2b, b2c) => {
      expect(getPrice(col(id), ci, 'igi', Y)).toBe(b2b);
      expect(getRetail(col(id), ci, 'igi', Y)).toBe(b2c);
    });
  });
});
