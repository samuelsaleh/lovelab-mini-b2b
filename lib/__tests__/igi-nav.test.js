...
describe("the certificate application's own nav", () => {
  ...
  // matching is a link at the foot of Models, and IGI's side is a link, not a screen.
  it('holds four screens, in the order the work actually happens', () => {
    // Sam, 16 Sept 2026: no Dashboard — Stock is the front page.
    ...
    expect(CERTIFICATE_NAV.map((n) => n.label)).toEqual(['Stock', 'Movements', 'Models', 'Invoices'])
  })

  it('has no group headings any more — four things need no grouping', () => {
  ...
  it('keeps the old addresses working', () => {
    ...
    // The front door opens on Stock; matching is its own quiet page again,
    // reached from the foot of Models (16 Sept 2026).
    expect(page('')).toContain("redirect('/certificates/stock')")
    expect(page('matching')).toContain('CertificatesMatchingClient')
