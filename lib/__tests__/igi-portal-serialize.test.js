...
describe('a LoveLab admin previewing IGI’s portal sees the portal, not a likeness', () => {
  const preview = fs.readFileSync(
    path.join(process.cwd(), 'app', 'api', 'igi', 'preview', '[screen]', 'route.js'), 'utf8')

  it('answers from the same builders IGI’s own routes use', () => {
    expect(preview).toContain('PORTAL_VIEWS')
    expect(preview).toContain('@/lib/igi/portalViews')
  })

  it('runs no query of its own', () => {
    // A .from() here is how "the same screen" quietly becomes "a similar
    // screen" a few months later.
    expect(preview).not.toMatch(/\.from\(/)
  })

  it('is LoveLab-only', () => {
    expect(preview).toContain('requireLoveLab')
    expect(preview).not.toContain('requireIgi')
  })

  it('offers no way to write', () => {
    // Reading IGI's screens is fair. Typing on their behalf is not: the record
    // is only worth something to both companies while each enters its own half.
    for (const verb of ['POST', 'PATCH', 'PUT', 'DELETE']) {
      expect(preview).not.toContain(`export async function ${verb}`)
    }
  })

  it('has a preview for every screen IGI have, and no more', () => {
    const { PORTAL_VIEWS } = require('../igi/portalViews')
    expect(Object.keys(PORTAL_VIEWS).sort()).toEqual(['history', 'invoices', 'stock', 'todo'])
  })
})
