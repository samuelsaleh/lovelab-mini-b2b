...
import {
  FORBIDDEN_TO_IGI, toIgiModel, toIgiLine, toIgiVisit, toIgiCount,
...
describe('IGI never receives a shelf figure', () => {
  const MODEL = {
    ...
    shape: 'Round', spec: null, order_min: 1800, sort_order: 3,
    // Fields that must not travel, present on the row IGI's own query can return.
    shelf_min: 25, qty_ordered: 12250, pool_min: 7,
  }

  it('sends a model with their stock and the level LoveLab want, and nothing else of LoveLab\'s', () => {
    // Sam, 16 Sept 2026: order_min is the one level, and IGI see it — as
    // "level", the minimum they must hold. The shelf level still never travels.
    const out = toIgiModel(MODEL, { pool: 11020, askedNow: 50 })
    expect(out).toEqual({
      id: 'm1', serial: 'LGAJ6530', name: 'Cuty-Cubix', stones: '1', carat: 0.1,
      shape: 'Round', spec: null, pool: 11020, level: 1800, asked_now: 50,
    })
    expect(out).not.toHaveProperty('shelf_min')
    expect(out).not.toHaveProperty('pool_min')
  })

  it('sends no level at all when LoveLab have not set one', () => {
    expect(toIgiModel({ ...MODEL, order_min: null }, { pool: 1, askedNow: 0 }).level).toBeNull()
  })
  ...
  it('tells IGI when a movement is a correction, and defaults to not', () => {
    const base = { id: 'v', visit_no: 30, visit_date: '2026-07-28', status: 'closed', date_suspect: false, unattributed_total: null }
    expect(toIgiVisit({ ...base, correction: true }, []).correction).toBe(true)
    expect(toIgiVisit(base, []).correction).toBe(false)
  })
  ...
  it('keeps every forbidden field out of every shape', () => {
    ...
    const payloads = [
      ...
      toIgiCount({ id: 'c', model_id: 'm1', was: 82, counted: 0, delta: -82, note: null, counted_at: '2026-09-18T10:00:00Z' }, MODEL),
    ]
