...
describe('notifyOrderEvent', () => {
  ...
  test('a server ORDER_NOTIFICATION_EMAILS setting cannot drop them: the extra is appended to whatever it says', async () => {
    process.env.ORDER_NOTIFICATION_EMAILS = 'albertosaleh@gmail.com, dionne@love-lab.com';
    await notifyOrderEvent(fakeAdmin(), { kind: 'created', document: sentOrder }, { sendEmail, recordHealthEvent });
    expect(sendEmail.mock.calls[0][0].to).toEqual(['albertosaleh@gmail.com', 'dionne@love-lab.com', 'debbie@love-lab.com', 'dahlia@love-lab.com']);
  });
