...
describe('notifyOrderEvent', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.EXTRA_ORDER_NOTIFICATION_EMAILS;
    delete process.env.SENDER_EMAIL;
  });

  // Sam, 15 Sep 2026: Debbie and Dahlia are on every order notice. (The 14 Sep
  // rule was B2C only; every order since was B2B, so Debbie heard nothing.)
  // Dionne is pinned the same way so ORDER_NOTIFICATION_EMAILS cannot drop her.
  const FIVE = ['alberto@love-lab.com', 'dionne@love-lab.com', 'elie@love-lab.com', 'debbie@love-lab.com', 'dahlia@love-lab.com'];

  test('every order notice also goes to Debbie and Dahlia, B2B and B2C alike', async () => {
    await notifyOrderEvent(fakeAdmin(), { kind: 'created', document: sentOrder }, { sendEmail, recordHealthEvent });
    expect(sendEmail.mock.calls[0][0].to).toEqual(FIVE);

    await notifyOrderEvent(fakeAdmin(), { kind: 'created', document: { ...sentOrder, order_channel: 'b2c' } }, { sendEmail, recordHealthEvent });
    expect(sendEmail.mock.calls[1][0].to).toEqual(FIVE);

    await notifyOrderEvent(fakeAdmin(), { kind: 'sent_to_client', document: sentOrder, recipient: 'client@x.com' }, { sendEmail, recordHealthEvent });
    expect(sendEmail.mock.calls[2][0].to).toEqual(FIVE);
  });

  test('a server ORDER_NOTIFICATION_EMAILS setting cannot drop Debbie, Dahlia, or Dionne', async () => {
    process.env.ORDER_NOTIFICATION_EMAILS = 'albertosaleh@gmail.com';
    await notifyOrderEvent(fakeAdmin(), { kind: 'created', document: sentOrder }, { sendEmail, recordHealthEvent });
  ...
  test('sent_to_client always includes Dionne even when the base list omits her', async () => {
    process.env.ORDER_NOTIFICATION_EMAILS = 'alberto@love-lab.com, elie@love-lab.com';
    await notifyOrderEvent(
      fakeAdmin(),
      { kind: 'sent_to_client', document: sentOrder, recipient: 'client@x.com' },
      { sendEmail, recordHealthEvent },
    );
    expect(sendEmail.mock.calls[0][0].to).toEqual([
      'alberto@love-lab.com',
      'elie@love-lab.com',
      'dionne@love-lab.com',
      'debbie@love-lab.com',
      'dahlia@love-lab.com',
    ]);
    expect(sendEmail.mock.calls[0][0].subject).toMatch(/emailed to client/i);
  });

  test('Dionne stays on sent_to_client even when EXTRA is empty or omits her', async () => {
    process.env.ORDER_NOTIFICATION_EMAILS = 'alberto@love-lab.com, elie@love-lab.com';
    process.env.EXTRA_ORDER_NOTIFICATION_EMAILS = '';
    await notifyOrderEvent(
      fakeAdmin(),
      { kind: 'sent_to_client', document: sentOrder, recipient: 'client@x.com' },
      { sendEmail, recordHealthEvent },
    ...
    process.env.EXTRA_ORDER_NOTIFICATION_EMAILS = 'debbie@love-lab.com, dahlia@love-lab.com';
    await notifyOrderEvent(
      fakeAdmin(),
      { kind: 'sent_to_client', document: sentOrder, recipient: 'client@x.com' },
      { sendEmail, recordHealthEvent },
  ...
  test('EXTRA_ORDER_NOTIFICATION_EMAILS overrides the extra, an empty value switches it off, duplicates are dropped', async () => {
    process.env.EXTRA_ORDER_NOTIFICATION_EMAILS = 'debbie@love-lab.com, Alberto@love-lab.com, x@love-lab.com';
    await notifyOrderEvent(fakeAdmin(), { kind: 'updated', document: sentOrder, replacedDocument: { created_at: '2026-09-01T10:00:00Z' } }, { sendEmail, recordHealthEvent });
    expect(sendEmail.mock.calls[0][0].to).toEqual(['alberto@love-lab.com', 'dionne@love-lab.com', 'elie@love-lab.com', 'debbie@love-lab.com', 'x@love-lab.com']);

    process.env.EXTRA_ORDER_NOTIFICATION_EMAILS = '';
    await notifyOrderEvent(fakeAdmin(), { kind: 'created', document: sentOrder }, { sendEmail, recordHealthEvent });
    expect(sendEmail.mock.calls[1][0].to).toEqual(['alberto@love-lab.com', 'dionne@love-lab.com', 'elie@love-lab.com']);
  });
