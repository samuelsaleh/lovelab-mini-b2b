...
describe('notifyOrderEvent', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.B2C_ORDER_NOTIFICATION_EMAILS;
    delete process.env.SENDER_EMAIL;
  });

  // Sam, 14 Sep 2026: Debbie only hears about B2C sales.
  test('a B2C order also goes to Debbie; a B2B order does not', async () => {
    await notifyOrderEvent(fakeAdmin(), { kind: 'created', document: { ...sentOrder, order_channel: 'b2c' } }, { sendEmail, recordHealthEvent });
    expect(sendEmail.mock.calls[0][0].to).toEqual(['alberto@love-lab.com', 'dionne@love-lab.com', 'elie@love-lab.com', 'debbie@love-lab.com']);
  ...
  test('B2C_ORDER_NOTIFICATION_EMAILS overrides the extra, an empty value switches it off, duplicates are dropped', async () => {
    const b2c = { ...sentOrder, order_channel: 'b2c' };
    process.env.B2C_ORDER_NOTIFICATION_EMAILS = 'debbie@love-lab.com, Alberto@love-lab.com, x@love-lab.com';
    await notifyOrderEvent(fakeAdmin(), { kind: 'updated', document: b2c, replacedDocument: { created_at: '2026-09-01T10:00:00Z' } }, { sendEmail, recordHealthEvent });
    ...
    process.env.B2C_ORDER_NOTIFICATION_EMAILS = '';
    await notifyOrderEvent(fakeAdmin(), { kind: 'sent_to_client', document: b2c, recipient: 'client@x.com' }, { sendEmail, recordHealthEvent });
    expect(sendEmail.mock.calls[1][0].to).toEqual(['alberto@love-lab.com', 'dionne@love-lab.com', 'elie@love-lab.com']);
  });
