...
describe('Shapy Shine necklace collection', () => {
  ...
  // B2B never moved, so it is still identical on both lists. Retail diverges
  // from 2026: the Aug 2026 reprice lifted the SSF 0.50 bracelet B2C to €600,
  // and 600 x 1.20 = 720 flows through the +20% rule.
  it('prices at SSF +20% (B2B exact, retail rounded up to 5)', () => {
    for (const year of ['2025', '2026']) {
      expect(SSF_NECK.carats.map((_, i) => getPrice(SSF_NECK, i, 'igi', year))).toEqual([66, 120, 186])
    }
    expect(SSF_NECK.carats.map((_, i) => getRetail(SSF_NECK, i, 'igi', '2025'))).toEqual([220, 400, 540])
    expect(SSF_NECK.carats.map((_, i) => getRetail(SSF_NECK, i, 'igi', '2026'))).toEqual([220, 400, 720])
  })
