...
describe('necklace prices (B2B / retail)', () => {
  for (const year of ['2025', '2026']) {
    it(`CUTY necklace ${year}`, () => {
      ...
      expect(CUTY_NECK.carats.map((_, i) => getRetail(CUTY_NECK, i, 'igi', year))).toEqual([194, 394, 538])
    })
...
describe('product type filtering', () => {
  ...
  it('getCollectionsByType("necklace") returns exactly the three necklaces', () => {
    const necks = getCollectionsByType(COLLECTIONS, 'necklace').map(c => c.id).sort()
    expect(necks).toEqual(['CUTY_NECK', 'M3_NECK', 'M4_NECK'])
  })
