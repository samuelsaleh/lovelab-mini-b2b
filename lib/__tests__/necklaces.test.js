...
describe('necklace collections', () => {
  ...
  it('restrict cord colours to the agreed palettes', () => {
    expect(CUTY_NECK.allowedColors).toEqual([
...
describe('Shapy Shine necklace collection', () => {
  ...
  it('has the right label, carats, sizes and cord, with no bezel/prong housing', () => {
    ...
    // No metal housing so the shapes are selectable directly after carat.
    expect(SSF_NECK.housing).toBeUndefined()
  ...
  it('restricts cord colours to the 11 CUTY-necklace colours', () => {
    expect(SSF_NECK.allowedColors).toEqual([
      'Orange', 'Light Blue', 'Black', 'Fluo Pink', 'Fluo Yellow',
      'Light Pink', 'Ivory', 'Red', 'Gold', 'Silver Grey', 'Green',
    ])
  })
  ...
  it('requires shape (not metal setting/colour) on the order row', () => {
    const col = findCollection('SHAPY SHINE NECKLACE')
    // Complete row: shape + size + cord colour, no bezel/prong metal needed.
    const complete = { quantity: '2', collection: 'SHAPY SHINE NECKLACE', carat: '0.10', unitPrice: '66', shape: 'Heart', size: 'S/M', colorCord: 'Red' }
    expect(validateRow(complete, col).ok).toBe(true)

    // Missing shape should flag the row; setting/bpColor must NOT be required.
    const noShape = { ...complete, shape: '' }
    const res = validateRow(noShape, col)
    expect(res.ok).toBe(false)
    expect(res.missing).toContain('shape')
    expect(res.missing).not.toContain('setting')
    expect(res.missing).not.toContain('bpColor')
  })
})
