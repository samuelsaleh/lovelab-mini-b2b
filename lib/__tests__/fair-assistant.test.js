...
describe('fair-assistant email chrome follows the lead language', () => {
  test('German lead gets German heading, pills and default buttons', () => {
    const html = renderFairOutreachEmail({
      siteUrl: 'https://app.example.com', greeting: 'Hallo Susanne,', headline: 'x',
      paragraph1: 'x', paragraph2: 'x', signoff: 'x', lang: 'de',
      button1: { label: 'Visit our website', url: 'https://www.lovelab.be' },
      button2: { label: 'B2B Login', url: 'https://b2b-lovelab.com' },
    });
    expect(html).toContain('Entdecken Sie unsere Kollektionen');
    expect(html).toContain('14 Tage Rückgaberecht');
    expect(html).toContain('Weltweiter Versand');
    expect(html).toContain('Kontakt');
    expect(html).toContain('Website besuchen');
    expect(html).toContain('B2B-Login');
    expect(html).not.toContain('EXPLORE OUR COLLECTIONS');
  });

  test('a custom button label is never translated', () => {
    const html = renderFairOutreachEmail({
      siteUrl: 'x', greeting: 'x', headline: 'x', paragraph1: 'x', paragraph2: 'x', signoff: 'x', lang: 'de',
      button1: { label: 'Voir le lookbook', url: 'u' }, button2: {},
    });
    expect(html).toContain('Voir le lookbook');
  });

  test('English stays English and unknown languages fall back to it', () => {
    const en = renderFairOutreachEmail({ siteUrl: 'x', greeting: 'x', headline: 'x', paragraph1: 'x', paragraph2: 'x', signoff: 'x' });
    expect(en).toContain('Explore our collections');
    const xx = renderFairOutreachEmail({ siteUrl: 'x', greeting: 'x', headline: 'x', paragraph1: 'x', paragraph2: 'x', signoff: 'x', lang: 'xx' });
    expect(xx).toContain('Explore our collections');
  });
});
