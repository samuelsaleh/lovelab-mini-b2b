...
describe('lib/email.js — isExcludedFromOrderCopies', () => {
  let isExcludedFromOrderCopies

  beforeEach(() => {
    jest.resetModules()
    ;({ isExcludedFromOrderCopies } = require('../email'))
  })

  it('excludes Sam personal Gmail regardless of case or spaces', () => {
    expect(isExcludedFromOrderCopies('samuelsaleh@gmail.com')).toBe(true)
    expect(isExcludedFromOrderCopies('  SamuelSaleh@Gmail.COM  ')).toBe(true)
    expect(isExcludedFromOrderCopies('sameworldsalad@gmail.com')).toBe(true)
    expect(isExcludedFromOrderCopies('  SameWorldSalad@Gmail.COM  ')).toBe(true)
  })

  it('does not exclude Alberto or office mailboxes', () => {
    expect(isExcludedFromOrderCopies('albertosaleh@gmail.com')).toBe(false)
    expect(isExcludedFromOrderCopies('dionne@love-lab.com')).toBe(false)
    expect(isExcludedFromOrderCopies('')).toBe(false)
    expect(isExcludedFromOrderCopies(null)).toBe(false)
  })
})
