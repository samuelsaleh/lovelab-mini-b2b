 ...
 *
 * The October 2026 price list is gated in the UI via canSeePricelist. October
 * now reprices both admin-preview collections (Moonlight / Sienna / Za-Ha) and
 * Iconix (Flower / Riviera / Linea), which every logged-in agent can sell. So
 * any agent who can sell a collection with a '2026-10' bucket must be offered
 * the October list — otherwise they would only ever see the older Iconix
 * numbers on the 2026 list.
 *
 * This suite pins that audience rule and the catalog invariant behind it: every
 * collection that carries a 2026-10 bucket must be one the October audience can
 * actually sell (admin-only or Iconix grant), so the list never "secretly"
 * changes a legacy SKU.
 */
...
describe('October list audience follows the collections it reprices', () => {
  test('every logged-in agent sees October (they all sell Iconix)', () => {
  ...
  test('unauthenticated surfaces still do not see October', () => {
