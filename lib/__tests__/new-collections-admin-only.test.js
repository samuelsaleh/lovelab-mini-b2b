...
describe('getVisibleCollections', () => {
  it('admins see every sellable collection', () => {
    ...
    expect(visible.map((c) => c.id)).not.toContain('SSRG')
    expect(visible).toHaveLength(COLLECTIONS.length - 1)
  })
  ...
  it('non-admins never see any preview collection', () => {
    ...
    expect(visibleIds).not.toContain('SSRG')
    // …but still see the other legacy ones (minus retired G/H).
    expect(visible).toHaveLength(COLLECTIONS.length - ADMIN_ONLY_IDS.length - 1)
  })
