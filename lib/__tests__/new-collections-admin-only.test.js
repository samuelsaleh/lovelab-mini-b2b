...
describe('AI system prompt gating', () => {
  ...
  // Riviera is braided-only (Sam, Sep 2026). The advisor must never ask a
  // closure for it, but the rule may only be spelled out to callers who can
  // see Riviera at all.
  it('tells admins and Iconix agents that Riviera is braided-only', () => {
    for (const opts of [{ includeAdminOnly: true }, { allowedPreviewIds: ICONIX_PREVIEW_COLLECTION_IDS }]) {
      const prompt = buildSystemPrompt('2026', opts)
      expect(prompt).toContain('Riviera Four and Riviera Eight are braided-only')
      expect(prompt).toContain('SHAPY SHINE FANCY and Riviera Four/Eight are always "braided"')
    }
  })

  it('keeps the Riviera closure rule out of the non-admin prompt', () => {
    const prompt = buildSystemPrompt('2026', { includeAdminOnly: false })
    expect(prompt).not.toContain('Riviera')
    expect(prompt).toContain('SHAPY SHINE FANCY are always "braided"')
  })
