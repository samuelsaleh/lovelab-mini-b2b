...
const NICOLAS = { email: 'nicolas.vial@ascension-france.com', role: 'agent' }
...
describe('getVisibleCollections — Bastian + Nicolas partial Iconix access', () => {
  it.each([BASTIAN, NICOLAS])('includes the 6 Iconix collections for %s', (profile) => {
  ...
  it.each([BASTIAN, NICOLAS])('excludes Moonlight, Sienna, and Za-Ha for %s', (profile) => {
    const visibleIds = getVisibleCollections(profile).map((c) => c.id)
