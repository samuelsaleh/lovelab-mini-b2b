describe('documentsHaveActivityAtColumn', () => {
  beforeEach(() => {
    jest.resetModules()
  })
  ...
  it('returns true when the schema probe succeeds', async () => {
    ...
    const { documentsHaveActivityAtColumn } = require('../activityAtColumn')

    await expect(documentsHaveActivityAtColumn(client)).resolves.toBe(true)
  ...
  it('returns false when the database reports a missing column', async () => {
    ...
    await expect(documentsHaveActivityAtColumn(client)).resolves.toBe(false)
  })
