...
describe('resolveCommissionAgent', () => {
  ...
  // Sam, 15 Sep 2026: an admin marked commercial is an agent for attribution.
  test('tier 1 — an admin who is also a commercial is attributable like any agent', async () => {
    const admin = makeAdmin({
      profiles: { maybeSingle: { data: { id: 'raphael', role: 'admin', is_agent: true, agent_status: 'active', commission_rate: 10 }, error: null } },
    });
    const r = await resolveCommissionAgent(admin, { id: 'd1', created_by: 'raphael', event_id: null });
    expect(r).toMatchObject({ agentId: 'raphael', via: 'creator' });
  });
...
describe('upsertCommissionForDocument', () => {
  ...
  test('document-linked commission inherits the original order date', async () => {
    const { admin, upsertCalls } = buildAdmin();
    await upsertCommissionForDocument(admin, {
      document: {
        id: 'inhorgenta-order',
        total_amount: 1000,
        created_at: '2026-02-21T11:40:53.615336+00:00',
      },
      profile: { commission_rate: 15, organization_id: null },
      agentId: 'silke',
    });

    expect(upsertCalls[0].row.created_at).toBe('2026-02-21T11:40:53.615Z');
  });

  test('invalid or missing document date leaves database timestamp fallback intact', async () => {
    const { admin, upsertCalls } = buildAdmin();
    await upsertCommissionForDocument(admin, {
      document: { id: 'legacy-order', total_amount: 1000, created_at: 'not-a-date' },
      profile: { commission_rate: 10, organization_id: null },
      agentId: 'a1',
    });

    expect(upsertCalls[0].row).not.toHaveProperty('created_at');
  });

  test('materialization mode uses ON CONFLICT DO NOTHING to preserve historical status', async () => {
    const { admin, upsertCalls } = buildAdmin();
    await upsertCommissionForDocument(admin, {
      document: { id: 'd1', total_amount: 1000 },
      profile: { commission_rate: 10, organization_id: null },
    ...
    expect(upsertCalls[0].opts).toEqual({
      onConflict: 'agent_id,document_id,type',
