...
describe('upsertCommissionForDocument', () => {
  function buildAdmin({
    ...
    const fromImpl = (table) => {
      ...
      if (table === 'agent_commissions') {
        ...
        chain.__setUpdateResult = () => {
          chain.then = (resolve) => Promise.resolve({ data: null, error: updateError }).then(resolve);
        };
    ...
    return { admin: { from: jest.fn(fromImpl) }, upsertCalls, updateCalls };
  }
  ...
  test('preserves an existing approved status when recalculating a commission', async () => {
    const { admin, upsertCalls } = buildAdmin({
      existingCommissions: [{ id: 'c1', agent_id: 'a1', status: 'approved' }],
    });
  ...
  test('does not reopen a paid commission on document re-save', async () => {
    const { admin, upsertCalls } = buildAdmin({
      existingCommissions: [{ id: 'c1', agent_id: 'a1', status: 'paid' }],
    });
    const r = await upsertCommissionForDocument(admin, {
      document: { id: 'd1', total_amount: 1000 },
      profile: { commission_rate: 10 },
    ...
    expect(r).toMatchObject({ skipped: true, reason: 'already_paid' });
    expect(upsertCalls).toEqual([]);
  });

  test('does not reopen a cancelled same-agent commission on document re-save', async () => {
    const { admin, upsertCalls } = buildAdmin({
      existingCommissions: [{ id: 'c1', agent_id: 'a1', status: 'cancelled' }],
    });
    const r = await upsertCommissionForDocument(admin, {
      document: { id: 'd1', total_amount: 1000 },
      profile: { commission_rate: 10 },
    ...
    expect(r).toMatchObject({ skipped: true, reason: 'already_cancelled' });
    expect(upsertCalls).toEqual([]);
  });

  test('cancels open commission rows for prior agents before inserting the new attribution', async () => {
    const { admin, upsertCalls, updateCalls } = buildAdmin({
      existingCommissions: [{ id: 'old', agent_id: 'agent-old', status: 'pending' }],
    });
    await upsertCommissionForDocument(admin, {
      document: { id: 'd1', total_amount: 1000 },
      profile: { commission_rate: 10 },
    ...
    expect(updateCalls[0].row).toMatchObject({
      status: 'cancelled',
      notes: expect.stringContaining('attribution moved'),
    });
    expect(upsertCalls[0].row).toMatchObject({
      agent_id: 'agent-new',
      document_id: 'd1',
