 ...
 * catalog must fall back to DEFAULT_PRICELIST ('2026-10'). This guarantees that
...
describe('catalog pricelist defaults', () => {
  test('PRICELISTS exposes all three lists and DEFAULT_PRICELIST is October', () => {
    expect(PRICELISTS).toEqual(['2025', '2026', '2026-10']);
    expect(DEFAULT_PRICELIST).toBe('2026-10');
  });

  test('every pricelist has a label to render in the toggle and the order form', () => {
    for (const year of PRICELISTS) {
      expect(typeof PRICELIST_LABELS[year]).toBe('string');
      expect(PRICELIST_LABELS[year].length).toBeGreaterThan(0);
    }
  });

  describe('resolvePricelist', () => {
    test('returns the default when input is missing', () => {
      expect(resolvePricelist(undefined)).toBe('2026-10');
      expect(resolvePricelist(null)).toBe('2026-10');
    });
    test('returns the default for an unknown year', () => {
      expect(resolvePricelist('2024')).toBe('2026-10');
      expect(resolvePricelist('something-bogus')).toBe('2026-10');
    });
    ...
    test('passes valid years through', () => {
      expect(resolvePricelist('2025')).toBe('2025');
      expect(resolvePricelist('2026')).toBe('2026');
      expect(resolvePricelist('2026-10')).toBe('2026-10');
    });
  });

  // A collection with no bucket for the requested list resolves through
  // PRICELIST_BASE. October carries only the collections it repriced, so the
  // chain is what keeps CUTY quotable at '2026-10'.
  describe('pricelist inheritance', () => {
    test('October inherits from 2026', () => {
      expect(PRICELIST_BASE['2026-10']).toBe('2026');
    });

    test('every base points at a real pricelist and terminates', () => {
      for (const [year, base] of Object.entries(PRICELIST_BASE)) {
        expect(PRICELISTS).toContain(year);
        expect(PRICELISTS).toContain(base);
        const seen = new Set();
        let cur = year;
        while (cur && !seen.has(cur)) {
          seen.add(cur);
          cur = PRICELIST_BASE[cur];
        }
        expect(cur).toBeUndefined();
      }
    });

    test('CUTY has no October bucket yet still quotes 2026 prices there', () => {
      expect(cuty.prices['2026-10']).toBeUndefined();
      expect(getPrice(cuty, 3, 'igi', '2026-10')).toBe(100);
      expect(getRetail(cuty, 3, 'igi', '2026-10')).toBe(430);
    });
  });

  describe('getPrice / getRetail without a year', () => {
    test('CUTY 0.30 IGI -> October inherits 2026 prices when no year is passed', () => {
  ...
  describe('calculateQuote default', () => {
    ...
    test('omitting opts.pricelistYear uses October (inherits 2026 prices) and reports it on the quote', () => {
