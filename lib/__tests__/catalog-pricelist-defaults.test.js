...
describe('catalog pricelist defaults', () => {
  test('PRICELISTS keeps all three lists (saved orders re-price on them) and DEFAULT_PRICELIST is 2026', () => {
    expect(PRICELISTS).toEqual(['2025', '2026', '2026-10']);
    expect(DEFAULT_PRICELIST).toBe('2026');
  });

  test('a new order starts on the October list (Sam, 15 Sep 2026)', () => {
    expect(NEW_ORDER_PRICELIST).toBe('2026-10');
    expect(PRICELISTS).toContain(NEW_ORDER_PRICELIST);
  });
