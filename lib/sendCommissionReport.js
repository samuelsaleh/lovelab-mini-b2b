...
export async function sendCommissionReportEmail({
  ...
  const ccList = parseRecipientList(cc, process.env.COMMISSION_REPORT_CC);
  const bccList = parseRecipientList(bcc, process.env.COMMISSION_REPORT_BCC);
  ...
  return { ...result, recipient: to, cc: ccList, bcc: bccList };
}

function parseRecipientList(explicit, envValue) {
      ...
      <tr>
        <td style="padding:20px 28px;">
          <p style="margin:0;font-size:13px;color:#666;line-height:1.7;">
            ...
            <strong>Reply to this email</strong> to reach ${escapeHtml(agent.name)} directly.
          </p>
        </td>
      </tr>
      ...
      <tr>
        <td style="padding:16px 28px 24px;border-top:1px solid #f0edf4;">
          <p style="margin:0;font-size:11px;color:#bbb;line-height:1.6;">
            Generated automatically by LoveLab B2B &middot; ${escapeHtml(period.label)}<br>
