// ─── HOUSING OPTIONS ───
// Matchy Fancy housing is always a two-metal code (first letter = housing of
// the first stone, second = the other), for Bezel AND Prong alike. The Lovelab
// ERP resolves the SKU from this code, so a bare metal name ("White") yields
// no SKU — never offer those (Sam, Sep 2026).
const MATCHY_HOUSING = [
...
export const HOUSING = {
  ...
  matchy: {
    bezel: MATCHY_HOUSING,
    prong: MATCHY_HOUSING,
  },
  // Keep flat aliases for backward compatibility
  matchyBezel: MATCHY_HOUSING,
  matchyProng: MATCHY_HOUSING,
...
// Iconix silk bracelets with a braided closure ship in three individual sizes
// (Sam, Sep 2026). Non-braided keeps the grouped silk sizes above.
export const SIZES_ICONIX_BRAIDED = ['S', 'M', 'L']
...
// A collection can opt out of the grouping with `sizesIgnoreClosure` (Multi
// Five): the Lovelab ERP only knows its SKUs in the individual XS–XL sizes, so a
// grouped S/M or L/XL line would come across without a SKU (Sam, Sep 2026).
export function sizeOptionsForClosure(col, closureType) {
  if (!col?.sizes) return []
  if (col.hasClosure && !col.sizesIgnoreClosure && resolveClosure(col, closureType) === 'nonBraided') return SIZES_SILK
  return col.sizes
}
...
// Shine is braided-only across all sizes (Alberto, Aug 2026); Riviera Four and
// Eight are braided-only too (Sam, Sep 2026).
...
// A collection may name the closure a row gets when nobody picked one.
// Za-Ha and the Flower pieces default to non-braided: every Iconix order saved
// before the closure existed carries a grouped silk size (S/M, L/XL), which is
// the non-braided list — so the default keeps those rows valid instead of
// blanking their size the next time they are opened. Riviera is forced braided
// instead (see getForcedClosure). CUTY has no default and still asks; CUBIX is
// forced non-braided.
export function getDefaultClosure(col) {
  if (!col?.hasClosure) return null
  return getForcedClosure(col) || col.defaultClosure || null
}

// The closure a row should carry: the forced value wins over whatever was
// stored, so old lines saved as non-braided normalise on the next edit; then
// the stored value; then the collection's default, if it has one.
export function resolveClosure(col, closureType) {
  if (!col?.hasClosure) return null
  return getForcedClosure(col) || closureType || col.defaultClosure || null
}
...
export const COLLECTIONS = [
  ...
  { id: 'CUBIX', label: 'CUBIX', carats: ['0.05', '0.10', '0.20'], certificate: 'both',
    ...
    // Non-braided only — closure picker hidden, every row pinned (same pattern
    // as Multi Four/Five braided-only). CUBIX already ships in silk grouped sizes.
    minC: 3, cord: 'nylon', housing: 'goldMetal', sizes: SIZES_SILK, hasClosure: true, forcedClosure: 'nonBraided' },
  { id: 'M3', label: 'MULTI THREE', carats: ['0.15', '0.30', '0.60', '0.90'], certificate: 'igi',
    ...
    // Braided only (Sam, 14 Sep 2026: "het kan alleen maar in braided") — the
    // closure picker is hidden and every row is pinned, like Shapy Shine.
    minC: 2, cord: 'nylon', housing: 'multiThree', sizes: SIZES_NYLON, hasClosure: true, forcedClosure: 'braided',
  ...
  { id: 'M4', label: 'MULTI FOUR', carats: ['0.20', '0.40'], certificate: 'igi',
    ...
    // Braided only — closure picker hidden, every row pinned (same as Multi Three).
    minC: 2, cord: 'nylon', housing: 'goldMetal', sizes: SIZES_NYLON, hasClosure: true, forcedClosure: 'braided',
    allowedColors: ['Bordeaux', 'Red', 'Gold', 'Silver Grey', 'Black', 'Navy Blue', 'Royal Blue'] },
  { id: 'M5', label: 'MULTI FIVE', carats: ['0.25', '0.50'], certificate: 'igi',
    ...
    // Braided only — picker hidden. sizesIgnoreClosure kept: ERP has no S/M or L/XL SKU.
    minC: 2, cord: 'nylon', housing: 'goldMetal', sizes: SIZES_NYLON, hasClosure: true, forcedClosure: 'braided', sizesIgnoreClosure: true,
    allowedColors: ['Bordeaux', 'Red', 'Gold', 'Silver Grey', 'Black', 'Navy Blue', 'Royal Blue'] },
  ...
  // 2025 list). The '2026-10' buckets come from the masters CSV (Sep 2026
  // bracelet reprice) + Pricelist_LoveLab_2026_October.pdf. October carries its
  // own bucket wherever bracelet B2B/B2C differs from 2026 — Moonlight, Sienna
  // (except SI1 kept), Za-Ha, Flower, Linea, Riviera. Necklaces and classics
  // inherit 2026 via PRICELIST_BASE. All IGI / 925 silver.
  ...
  // exist: Long lost 0.20, Multi lost 0.15/0.30 and gained 0.70/1.01, Original
  // gained 0.20 and its 1 ct is spelled 1.01 ct. Multi's top size is now 1.01
  // (was 1.10). Sizes October introduced are null on 2025/2026 so they can
  ...
  { id: 'MFM', label: 'Original Moonlight', carats: ['0.10', '0.20', '0.30', '0.50', '0.70', '1.01'], certificate: 'igi',
    prices: {
      ...
      '2026-10': { igi: [70, 90, 120, 250, 300, 460] },
    },
    retail: {
      '2025': { igi: [170, null, 315, 675, 900, 1200] },
      '2026': { igi: [170, null, 315, 675, 900, 1200] },
      '2026-10': { igi: [210, 270, 360, 750, 900, 1380] },
    },
  ...
  { id: 'MNO', label: 'Long Moonlight', carats: ['0.05', '0.10', '0.30'], certificate: 'igi',
    prices: {
      '2025': { igi: [56, 68, 105] },
      '2026': { igi: [56, 68, 105] },
      '2026-10': { igi: [67, 85, 120] },
    },
    retail: {
      '2025': { igi: [170, 205, 315] },
      '2026': { igi: [170, 205, 315] },
      '2026-10': { igi: [255, 255, 360] },
    },
    minC: 2, cord: 'nylon', housing: 'metalEight', sizes: SIZES_NYLON,
    allowedColors: ['Silver Grey', 'Black', 'Bordeaux', 'Brown'] },
  { id: 'MNH', label: 'Multi Moonlight', carats: ['0.20', '0.40', '0.70', '1.01'], certificate: 'igi',
    prices: {
      '2025': { igi: [75, 130, null, null] },
      '2026': { igi: [75, 130, null, null] },
      '2026-10': { igi: [100, 165, 200, 320] },
    },
    retail: {
      '2025': { igi: [215, 390, null, null] },
      '2026': { igi: [215, 390, null, null] },
      '2026-10': { igi: [300, 495, 600, 960] },
    },
    minC: 2, cord: 'nylon', housing: 'metalEight', sizes: SIZES_NYLON,
    allowedColors: ['Silver Grey', 'Black', 'Bordeaux', 'Brown'] },

  // SIENNA — silk, 8 housing tiles (3 gold shiny + 5 matte)
  // Repriced on October except Sienna One (kept prior Oct numbers).
  ...
  { id: 'ZAHA', label: 'Za-Ha', carats: ['0.30'], certificate: 'igi',
    ...
    minC: 2, cord: 'silk', housing: 'metalThree', sizes: SIZES_ICONIX_BRAIDED, thicknessOptions: THIN_ONLY, hasClosure: true, defaultClosure: 'nonBraided',
    allowedColors: ['Silver grey', 'Black', 'Bordeaux', 'Brown'] },
  { id: 'LUVA', label: 'Flower Heart', carats: ['0.40'], certificate: 'igi',
    // 2026 B2B rounded per the official PDF (2026-07-19): 149.50 → 150.
    // October bracelet reprice (masters CSV) adds its own bucket.
    prices: {
      '2025': { igi: [150] },
      '2026': { igi: [150] },
      '2026-10': { igi: [170] },
  ...
  { id: 'LUMA', label: 'Flower Marquise', carats: ['0.40'], certificate: 'igi',
    ...
    minC: 2, cord: 'silk', housing: 'metalThree', sizes: SIZES_ICONIX_BRAIDED, thicknessOptions: THIN_ONLY, hasClosure: true, defaultClosure: 'nonBraided',
    allowedColors: ['Silver grey', 'Gold', 'Black', 'Bordeaux', 'Brown'] },
  // Riviera is braided-only (Sam, Sep 2026): no closure picker, every row is
  // pinned to braided and ships in S / M / L. Unlike the other Iconix silk
  // pieces there is no non-braided option and therefore no grouped size.
  { id: 'RIV4', label: 'Riviera Four', carats: ['0.20', '0.40'], certificate: 'igi',
    ...
    minC: 2, cord: 'silk', housing: 'metalThree', sizes: SIZES_ICONIX_BRAIDED, thicknessOptions: THIN_ONLY, hasClosure: true, forcedClosure: 'braided',
    allowedColors: ['Silver grey', 'Gold', 'Black', 'Bordeaux', 'Brown'] },
  { id: 'RIV8', label: 'Riviera Eight', carats: ['0.40', '0.80'], certificate: 'igi',
    // 2026 B2B corrected per the official PDF (2026-07-19): 130/175 → 115/150.
    prices: {
      '2025': { igi: [115, 150] },
      '2026': { igi: [115, 150] },
      '2026-10': { igi: [140, 175] },
...
// Moonlight Multi 0.70 / 1.01 and Moonlight Original 0.20, which are priced
...
export function calculateQuote(lines, opts = {}) {
  ...
  for (const l of lines) {
    ...
    for (const cfg of configs) {
      ...
      // Sizes differ between pricelists — Moonlight Multi 0.70 / 1.01 and
...
export const CORD_TYPE_LABELS = {
  ...
  nylon: 'Nylon',
  // Shapy Shine's thread has its own colour palette (the 'shine' key) but it
  // is a nylon thread, and the order form and PDF say what it is made of, not
  // which palette it draws from (14 Sep 2026: "het moet of nylon of silk zijn").
  shine: 'Nylon',
}
