...
// Closure-driven size options for the bracelet-thread collections (CUTY,
// CUBIX). Braided bracelets ship in the collection's regular individual sizes;
// non-braided bracelets only come in the grouped silk sizes (S/M, L/XL).
// Collections without `hasClosure` are unaffected and always return col.sizes.
...
export const COLLECTIONS = [
  ...
  // to the 5 colours Silver Grey / Gold / Black / Bordeaux / Brown via
  // allowedColors — note the casing differs between nylon ('Silver Grey') and
  // silk ('Silver grey') to match each CORD_COLORS palette exactly.

  // MOONLIGHT — nylon, 8 housing tiles (3 gold shiny + 5 matte)
  { id: 'MFM', label: 'Original Moonlight', carats: ['0.10', '0.30', '0.50', '0.70', '1.01'], certificate: 'igi',
    prices: {
  ...
  { id: 'CUTY_NECK', label: 'CUTY NECKLACE', productType: 'necklace', carats: ['0.10', '0.20', '0.30'], certificate: 'igi',
    ...
    allowedColors: ['Orange', 'Light Blue', 'Black', 'Fluo Pink', 'Fluo Yellow', 'Light Pink', 'Ivory', 'Red', 'Gold', 'Silver Grey', 'Green'] },
  ...
  // and the 11 CUTY-necklace cord colours. Prices = SSF 2026 +20% (B2B exact,
  // retail rounded up to nearest 5), identical across both pricelist years.
  // No bezel/prong metal housing (unlike the SSF bracelet) so the 7 shapes are
  // selectable directly after carat — the config is carat -> shape -> colour/size.
  { id: 'SSF_NECK', label: 'SHAPY SHINE NECKLACE', productType: 'necklace', carats: ['0.10', '0.30', '0.50'], certificate: 'igi',
    ...
    minC: 2, cord: 'shine', shapes: SHAPES_SHAPY_SHINE, sizes: SIZES_NECKLACE,
    allowedColors: ['Orange', 'Light Blue', 'Black', 'Fluo Pink', 'Fluo Yellow', 'Light Pink', 'Ivory', 'Red', 'Gold', 'Silver Grey', 'Green'] },
]
...
// Collections a given user is allowed to see/pick. Admins see everything;
// everyone else only sees the non-preview (legacy) collections.
export function getVisibleCollections(isAdmin) {
  return isAdmin
    ? COLLECTIONS
    : COLLECTIONS.filter((c) => !ADMIN_ONLY_COLLECTION_IDS.has(c.id))
}
