...
/**
 * POST /api/certificate-master — upsert B2B stock labels into ERP masters.
 * @param {{ names: string[] } | { items: Array<{ name: string }> }} payload
 */
export async function upsertCertificateMasters(payload) {
  const json = await lovelabFetch('certificate-master', { method: 'POST', body: payload })
  if (!json || json.success !== true) {
    throw new Error(json?.message || 'certificate-master upsert failed')
  }
  return json
}
