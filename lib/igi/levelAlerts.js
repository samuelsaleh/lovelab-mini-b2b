...
export async function loadModelsWithStatus(adminSupabase) {
  const [models, batches, lines, snapshots, counts, certIns, certOuts] = await Promise.all([
    adminSupabase.from('igi_models')
      .select('id, serial, name, stones, carat, shape, spec, state, shelf_min, shelf_opening, order_min, shelf_alerted_at, order_alerted_at, sort_order')
...
export async function runLevelAlerts(adminSupabase, deps = {}) {
  ...
  const recipients = [...new Set([
    ...(deps.recipients || [...getOrderNotificationRecipients(), ...getAdminNotificationRecipients().all]),
  ].filter(Boolean))]
