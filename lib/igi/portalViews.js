...
import { toIgiModel, toIgiLine, toIgiVisit, toIgiNewModel, toIgiCount } from './portalShapes'
 ...
 * since 10 Sept 2026 the models LoveLab added that still need a serial, and
 * since 16 Sept 2026 the models below the level LoveLab want IGI to hold.
 */
export function todoView(world) {
  const open = world.visits.filter((v) => v.status === 'requested')
  const produce = world.models
    .map((m) => toIgiModel(m, { pool: world.poolFor(m.id), askedNow: world.askedFor(m.id) }))
    .filter((m) => m.level != null && m.pool < m.level)
    .map((m) => ({ ...m, short_by: m.level - m.pool }))
  ...
  return {
    ...
    new_models: (world.awaiting || []).map(toIgiNewModel),
    produce,
  }
}

/**
 * What IGI hold, model by model, with the level LoveLab want and "asked right
 * now" beside it — their order book, which unlike a shelf figure says nothing
 * about how fast anything sells.
 */
...
/**
 * What has already happened: movements, the batches IGI recorded making, and
 * the counts they corrected (18 Sept 2026).
 */
export function historyView(world) {
  return {
    visits: world.visits.map((v) => ({
      ...
      correction: Boolean(v.correction),
    ...
    counts: (world.counts || [])
      .map((c) => toIgiCount(c, world.modelById.get(c.model_id)))
      .sort((a, b) => String(b.counted_at).localeCompare(String(a.counted_at))),
  }
}
