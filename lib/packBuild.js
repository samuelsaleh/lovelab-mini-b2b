...
function splitHousing(housing) {
  ...
  if (housing === 'Prong' || housing === 'Bezel') return { setting: housing, color: '' }
  return { setting: '', color: housing }
}

function certLabelFor(col, certType, caratIdx, pricelistYear) {
...
export function linesToFormRows(lines, { pricelistYear } = {}) {
  ...
  for (const line of lines) {
    ...
    for (const cfg of (line.colorConfigs || [])) {
      ...
      const certType = cfg.certType || getDefaultCert(col, cfg.caratIdx, pricelistYear)
      ...
      rows.push({
        ...
        cert: certLabelFor(col, certType, cfg.caratIdx, pricelistYear),
