...
export function getMainNavItems(profile) {
  ...
  return [
    ...
    { id: 'documents', label: 'Documents' },
    // A commercial admin (Sam, 15 Sep 2026) reaches their own orders,
    // commissions and payments from here too, not only from the admin portal.
    ...(isAdmin && profile?.is_agent && profile?.agent_status !== 'inactive'
      ? [{ id: 'my-sales', label: 'My Sales', href: '/admin/my-sales' }]
      : []),
  ]
}
...
/** The certificate application's own nav.
 *
 * Five screens. Sam, 10 Sept 2026: "why is it so complex?" took it from nine
 * to five; Sam, 16 Sept 2026: "too much information" emptied the Dashboard
 * down to two lists — what to collect, what to order at IGI — and nothing
 * else. Stock is one line per model; the request is a column on it. The
 * day-by-day view is a switch on Movements, matching is a link at the foot of
 * Models, and IGI's side is a link in the sidebar foot, not a screen of its
 * own. The old addresses still redirect.
 *
 * This is a separate application with its own shell, so it does not appear in
 * the admin sidebar — ADMIN_NAV_ITEMS carries a single door into it.
 */
export const CERTIFICATE_NAV = [
  { id: 'dashboard', label: 'Dashboard', href: '/certificates' },
  { id: 'stock',     label: 'Stock',     href: '/certificates/stock' },
  { id: 'visits',    label: 'Movements', href: '/certificates/visits' },
  { id: 'models',    label: 'Models',    href: '/certificates/models' },
  { id: 'invoices',  label: 'Invoices',  href: '/certificates/invoices' },
]

/** Where the old certificate addresses go now. */
export const CERTIFICATE_REDIRECTS = {
...
/**
 * Admin portal nav for a given profile. An admin who is also a commercial
 * (is_agent — Sam, 15 Sep 2026) gets "My Sales": their own page inside the
 * admin portal with the orders credited to them, their commissions and
 * payments. Not the agent portal — a commercial is an employee, not an agent.
 */
export function getAdminNavItems(profile) {
  if (!profile?.is_agent || profile?.agent_status === 'inactive') return ADMIN_NAV_ITEMS
  const items = [...ADMIN_NAV_ITEMS]
  const at = items.findIndex((i) => i.id === 'sales-team') + 1
  items.splice(at, 0, { id: 'my-sales', label: 'My Sales', href: '/admin/my-sales' })
  return items
}
