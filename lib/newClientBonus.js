...
/**
 * Forward-going hook called from lib/commissionAttribution.js after an
 * order commission is upserted. Creates a single new_client_bonus row
 * if and only if:
 *   - profile.new_client_bonus_enabled is true
...
export async function maybeCreateBonusForOrder(adminSupabase, { agentId, profile, document }) {
  ...
  const { error } = await adminSupabase.from('agent_commissions').insert({
    ...
    commission_amount: amt,
    status: 'pending',
    notes: `New client: ${docForCheck.client_company || docForCheck.client_name || '(unknown)'} — auto-detected`,
  });

  if (error) {
    const enriched = new Error(error.message || 'maybeCreateBonusForOrder insert failed');
    enriched.code = error.code || null;
    throw enriched;
  }

  return { created: true, amount: amt };
}
