...
export async function resolveCommissionAgent(adminSupabase, document) {
  ...
  // Sam, 22 Sep 2026: an admin took an order away from its agent ("No agent"
  // in the save dialog). A null agent_id alone cannot say that — for older
  // rows it means "not backfilled, fall back to the creator / the folder" —
  // so the explicit choice is stored as metadata.no_agent and wins here.
  if (document?.metadata?.no_agent === true) return null;
...
/**
 * Drop the pending order commission(s) of a document whose agent was removed.
 * Paid rows are money already moved and cancelled rows are audit history;
 * both stay. Never throws — the caller's health event covers a DB error.
 *
 * @returns {Promise<{ deleted: number }>}
 */
export async function removePendingOrderCommissions(adminSupabase, documentId) {
  if (!documentId) return { deleted: 0 };
