...
export async function upsertCommissionForDocument(
  ...
  if (existingErr) {
    const enriched = new Error(existingErr.message || 'Commission lookup failed');
    enriched.code = existingErr.code || null;
    enriched.details = existingErr.details || null;
    throw enriched;
  }

  const commissions = Array.isArray(existingRows) ? existingRows : [];
  if (commissions.some((row) => row?.status === 'paid')) {
    return { skipped: true, reason: 'already_paid' };
  }

  const sameAgentCommission = commissions.find((row) => row?.agent_id === agentId);
  const otherOpenCommissions = commissions.filter((row) =>
    row?.agent_id &&
  ...
  if (otherOpenCommissions.length > 0) {
    const { error: cancelErr } = await adminSupabase
      .from('agent_commissions')
      .update({
        status: 'cancelled',
        notes: 'Auto-cancelled because commission attribution moved to another agent.',
      })
      .eq('document_id', document.id)
      .eq('type', 'order')
    ...
    if (cancelErr) {
      const enriched = new Error(cancelErr.message || 'Commission attribution cleanup failed');
      enriched.code = cancelErr.code || null;
      enriched.details = cancelErr.details || null;
      throw enriched;
    }
  }

  if (sameAgentCommission?.status === 'cancelled') {
    ...
    .upsert(
      {
        ...
        status: sameAgentCommission?.status || 'pending',
      },
