...
const NON_COMMISSIONABLE_ORDER_CHANNELS = new Set(['internal', 'consignment', 'delete_from_stock']);

export function isCommissionableDocument(document) {
  const total = Number(document?.total_amount);
  return (
    document?.document_type === 'order' &&
    Number.isFinite(total) &&
    total > 0 &&
    !NON_COMMISSIONABLE_ORDER_CHANNELS.has(document?.order_channel)
  );
}

export async function cancelNonPayableCommissionsForDocument(adminSupabase, documentId, notes) {
  if (!documentId) {
