...
import { exactCollection, matchCollectionLabel } from './collectionMatch'
...
const CONDITIONAL_REQUIRED_ORDER = ['shape', 'setting', 'bpColor', 'closure', 'size', 'material', 'colorCord']
...
function collectionHasSetting(collection) {
  ...
  return ['shapyShine', 'matchy', 'sparkleProng', 'sparkleProngBezel', 'multiThree'].includes(collection.housing)
}
...
function collectionRequiresBpColor(collection) {
  // Sparkle housings render bpColor as N/A — Prong/Bezel is the setting, not a metal colour.
  return !!collection?.housing && collection.housing !== 'sparkleProng' && collection.housing !== 'sparkleProngBezel'
}
...
// Bracelet thread closure: explicitly OPTIONAL across the board.
//
// Originally we required this for CUTY / CUBIX (the two collections that
// opt-in via `hasClosure`). Sam reverted that decision on 2026-05-13:
// agents often build large CUTY orders where the closure choice is made
// later with the client, and getting "Row 1..80: missing closure" 80
// times in the warning banner buries the genuinely missing fields
// (carat, color, etc.).
//
// The closure dropdown is still rendered for CUTY/CUBIX rows so anyone
// who wants to pick "Braided" or "Non-braided" upfront can do so, but
// leaving it empty no longer flags the order as incomplete. Non-closure
// collections continue to render the N/A em-dash and were never required
// in the first place.
function collectionRequiresClosure(_collection) {
  return false
}
...
export function getRequiredFieldsForRow(collection) {
  ...
  for (const key of CONDITIONAL_REQUIRED_ORDER) {
    ...
    if (key === 'closure' && collectionRequiresClosure(collection)) required.push(key)
...
export function findCollection(productName) {
  ...
  if (!str) return null
  const exact = exactCollection(str)
  if (exact) return exact
  const aliasedLabel = matchCollectionLabel(str)
  if (aliasedLabel) return COLLECTIONS.find((c) => c.label === aliasedLabel) || null
...
export function splitHousing(housing) {
  ...
  if (str === 'Prong' || str === 'Bezel') return { setting: str, color: '' }
