...
function bump(list, name, hex, qty, revenue) {
  const key = String(name).trim().toLowerCase()
  if (!key) return
  let row = list.find((r) => r.name.toLowerCase() === key)
  if (!row) {
    row = { name, hex: hex || null, qty: 0, revenue: 0, catalog: false }
    list.push(row)
  }
  row.qty += qty
...
export function buildColorBreakdown(docs) {
  ...
  for (const d of eligibleDocs(docs)) {
    for (const row of d.metadata?.formState?.rows || []) {
      ...
      const snapped = normalizeCordColorName(col, cordType, rawColor) || rawColor
      ...
      if (group === 'nylon') {
        ...
        bump(nylon, placed.name, placed.hex, qty, revenue)
      } else if (group === 'silk') {
        const placed = snapToGroupPalette('silk', snapped, col, cordType)
        bump(silk, placed.name, placed.hex, qty, revenue)
      } else {
        bump(other, snapped, null, qty, revenue)
      }
    }
  }
