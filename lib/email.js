...
// since was B2B, so she heard nothing.) Dionne is pinned the same way so a
// server ORDER_NOTIFICATION_EMAILS override cannot drop her from
// "Order emailed to client" (and the other order notices). These are
// APPENDED to whatever ORDER_NOTIFICATION_EMAILS says. Override with
// EXTRA_ORDER_NOTIFICATION_EMAILS (comma-separated); an empty value
// switches the extra off.
const ALWAYS_ORDER_NOTIFICATION_EXTRA = [
  'dionne@love-lab.com',
  'debbie@love-lab.com',
  'dahlia@love-lab.com',
];
 ...
 *  Dionne is guaranteed last-pass so neither ORDER_NOTIFICATION_EMAILS nor an
 *  EXTRA_ORDER_NOTIFICATION_EMAILS override can leave her out of
 *  "Order emailed to client" (or the other order notices). */
export function getOrderNotificationRecipientsFor(_document) {
  ...
  const dionne = 'dionne@love-lab.com';
  if (!seen.has(dionne)) merged.push(dionne);
  return merged;
}
