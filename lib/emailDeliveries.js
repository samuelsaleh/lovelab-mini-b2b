...
export function _resetEmailDeliveriesProbe() { tableCache = null; opensCache = null; }

// Opens / clicks columns (supabase/migrations/20260915130000_email_opens.sql).
// Same idea: without the migration, opens are still counted as "delivered".
let opensCache = null;
export async function emailOpensAvailable(adminSupabase) {
  if (opensCache !== null) return opensCache;
  try {
    if (!(await emailDeliveriesAvailable(adminSupabase))) { opensCache = false; return false; }
    const query = adminSupabase.from('email_deliveries').select('opened_at');
    if (!query || typeof query.limit !== 'function') { opensCache = false; return false; }
    const { error } = await query.limit(1);
    opensCache = !error;
  } catch {
    opensCache = false;
...
async function mirrorToDraft(adminSupabase, row, { withOpens = false } = {}) {
  ...
  if (withOpens) {
    payload.opened_at = row.opened_at || null;
    payload.clicked_at = row.clicked_at || null;
  }
...
function labelForKind(kind) {
  switch (kind) {
    ...
    case 'price_list_announcement': return 'Price list announcement';
...
/**
 * An open or a click (Sam, 15 Sep 2026). Keeps the first time and a counter
 * on the email, mirrors them to the fair draft, and marks the email delivered
 * unless it already ended badly. Without the opens migration it only marks
 * delivered, as before. Never throws.
 */
export async function applyEngagement(adminSupabase, { resendId, kind, eventAt = null }, deps = {}) {
  if (!resendId || (kind !== 'opened' && kind !== 'clicked')) return { ok: false, reason: 'bad_input' };
  try {
    if (!(await emailDeliveriesAvailable(adminSupabase))) return { ok: false, reason: 'table_missing' };
    const withOpens = await emailOpensAvailable(adminSupabase);
    const { data: existing } = await adminSupabase
      .from('email_deliveries')
      .select('*')
      .eq('resend_id', resendId)
    ...
    const at = eventAt || new Date().toISOString();
    const bad = BAD_STATUSES.has(existing.status);
    const patch = {};
    if (!bad && existing.status !== 'delivered') {
      patch.status = 'delivered';
      patch.advice = explainDelivery('delivered').advice;
      patch.detail = null;
    }
    const lastTime = existing.last_event_at ? new Date(existing.last_event_at).getTime() : 0;
    if (new Date(at).getTime() >= lastTime) patch.last_event_at = at;
    if (withOpens) {
      // A click is an open too.
      if (!existing.opened_at) patch.opened_at = at;
      patch.open_count = (existing.open_count || 0) + (kind === 'opened' ? 1 : 0);
      if (kind === 'clicked') {
        if (!existing.clicked_at) patch.clicked_at = at;
    ...
    const row = updated || { ...existing, ...patch };
    await mirrorToDocument(adminSupabase, row);
    await mirrorToDraft(adminSupabase, row, { withOpens });
    return { ok: true, status: row.status, kind, changed: patch.status !== undefined, opened_at: row.opened_at || null, clicked_at: row.clicked_at || null };
  } catch (err) {
    console.error('[emailDeliveries] applyEngagement failed:', err?.message);
    return { ok: false, reason: 'error', error: err?.message };
  }
}
...
export async function applyResendEvent(adminSupabase, event, deps = {}) {
  ...
  if (type === 'email.opened' || type === 'email.clicked') {
    return applyEngagement(adminSupabase, {
      resendId: data.email_id, kind: type.replace('email.', ''), eventAt: event.created_at || data.created_at || null,
    }, deps);
  }
...
export async function refreshDelivery(adminSupabase, resendId, deps = {}) {
  ...
  const eventName = String(lastEvent || '').replace(/^email\./, '');
  if (eventName === 'opened' || eventName === 'clicked') {
    const res = await applyEngagement(adminSupabase, { resendId, kind: eventName, eventAt: null }, deps);
    ...
    return res.ok ? { ...res, status: 'delivered' } : res;
  }
  if (status === 'unknown') {
    if (await emailDeliveriesAvailable(adminSupabase)) {
      await adminSupabase.from('email_deliveries').update({ checked_at: now }).eq('resend_id', resendId);
