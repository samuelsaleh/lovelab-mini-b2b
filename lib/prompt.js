...
const PRODUCT_NAMES_LEGACY = 'CUTY, CUBIX, MULTI THREE, MULTI FOUR, MULTI FIVE, MATCHY FANCY, SHAPY SHINE FANCY, SHAPY SPARKLE FANCY, SHAPY SPARKLE RND G/H, SHAPY SPARKLE RND D VVS, HOLY (D VVS), CUTY NECKLACE, MULTI THREE NECKLACE, MULTI FOUR NECKLACE';
...
const buildHeader = (includeAdminOnly) => `You are a B2B order advisor chatbot for LoveLab Antwerp (Munich 2026).
...
7. **Closure (CUTY and CUBIX only)** — these bracelets ship with a thread closure that must be either Braided or Non-braided. If the user didn't specify, you MUST ask. Choices: ["Braided", "Non-braided"]. The closureType field on the line must be "braided" or "nonBraided".
...
- In quote JSON, each line's "product" MUST be exactly one of these labels (match spelling/case/spaces):
  ${includeAdminOnly ? `${PRODUCT_NAMES_LEGACY}, ${PRODUCT_NAMES_NEW}` : PRODUCT_NAMES_LEGACY}
...
const buildTail = (includeAdminOnly) => `COLORS:
...
HOLY: Brown,Grey,Green,Ivory,Royal Blue,Pink,Black,Red${includeAdminOnly ? '\nNEW COLLECTIONS (Moonlight nylon; Sienna & Iconix silk): Silver Grey, Gold, Black, Bordeaux, Brown (these 5 only)' : ''}
...
- SHAPY SPARKLE collections: no housing options${includeAdminOnly ? `
...
- ICONIX (Za-Ha, Flower Heart, Flower Marquise, Riviera Four/Eight, Linea Three/Five): shiny only — Yellow, White, Pink. Default: Yellow.` : ''}

SIZES:
- NYLON bracelets (CUTY, MULTI THREE/FOUR/FIVE, MATCHY FANCY, SHAPY SHINE FANCY, HOLY${includeAdminOnly ? ', Moonlight, Linea Three/Five' : ''}): XS, S, M, L, XL. Default: M.
- SILK bracelets (CUBIX, SHAPY SPARKLE collections${includeAdminOnly ? ', Sienna One–Five, Za-Ha, Flower Heart, Flower Marquise, Riviera Four/Eight' : ''}): S/M, L/XL. Default: S/M.

CORD OPTIONS:
- SHAPY SPARKLE RND G/H and SHAPY SPARKLE RND D VVS: Can have Silk OR Braided cord. Braided uses nylon colors.
- Silk cords have thickness options: Thin or Thick.${includeAdminOnly ? `
- The new silk collections (Sienna One–Five, Za-Ha, Flower Heart, Flower Marquise, Riviera Four/Eight) only come in Thin (no Thick option).` : ''}
...
- closureType ("braided" or "nonBraided") when the product is CUTY or CUBIX — must be specified by user, never assume
...
export function buildPricesBlock(pricelistYear, includeAdminOnly = true) {
  const yr = resolvePricelist(pricelistYear);
  const rows = ['PRICES (B2B / retail) — format: carat=€B2B/€retail'];
  for (const cfg of PROMPT_LINES) {
    // Preview (admin-only) collections are excluded for non-admins.
    if (!includeAdminOnly && ADMIN_ONLY_COLLECTION_IDS.has(cfg.id)) continue;
...
export function buildSystemPrompt(pricelistYear = DEFAULT_PRICELIST, { includeAdminOnly = true } = {}) {
  const yr = resolvePricelist(pricelistYear);
  return `${buildHeader(includeAdminOnly)}

${buildPricesBlock(yr, includeAdminOnly)}

${buildTail(includeAdminOnly)}`;
}
