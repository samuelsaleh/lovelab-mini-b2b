...
7. **Closure (nylon bracelets — CUTY, CUBIX, MULTI FOUR/FIVE, MATCHY FANCY, HOLY)** — these nylon-thread bracelets ship with a thread closure that must be either Braided or Non-braided. If the user didn't specify, you MUST ask. Choices: ["Braided", "Non-braided"]. The closureType field on the line must be "braided" or "nonBraided". Note: a Non-braided closure only comes in the grouped sizes S/M and L/XL (Braided keeps the full XS–XL range) — except MULTI FIVE, which is always XS, S, M, L, XL whatever the closure (never S/M or L/XL). MULTI THREE and SHAPY SHINE FANCY are braided-only — never ask, always emit closureType "braided".${rivieraClosureNote} Silk bracelets (Shapy Sparkle) and necklaces have NO closure.
...
- MATCHY FANCY: Bezel OR Prong, housing always a two-metal code: WW, YY, PP, WY, WP, YP (never a bare "White" / "Yellow" / "Pink")
...
SIZES:
- NYLON bracelets (CUTY, MULTI THREE/FOUR/FIVE, MATCHY FANCY, SHAPY SHINE FANCY, HOLY${lineaNylonExtra}): XS, S, M, L, XL. Default: M. MULTI FIVE is always one of these five, even non-braided.
