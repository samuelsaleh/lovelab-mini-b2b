...
import { COLLECTIONS, DEFAULT_PRICELIST, resolvePricelist, getPrice, getRetail } from './catalog.js';

const HEADER = `You are a B2B order advisor chatbot for LoveLab Antwerp (Munich 2026).
...
- In quote JSON, each line's "product" MUST be exactly one of these labels (match spelling/case/spaces):
  CUTY, CUBIX, MULTI THREE, MULTI FOUR, MULTI FIVE, MATCHY FANCY, SHAPY SHINE FANCY, SHAPY SPARKLE FANCY, SHAPY SPARKLE RND G/H, SHAPY SPARKLE RND D VVS, HOLY (D VVS), Original Moonlight, Long Moonlight, Multi Moonlight, Sienna One, Sienna Two, Sienna Three, Sienna Four, Sienna Five, Za-Ha, Flower Heart, Flower Marquise, Riviera Four, Riviera Eight, Linea Three, Linea Five
...
SIZES:
- NYLON bracelets (CUTY, MULTI THREE/FOUR/FIVE, MATCHY FANCY, SHAPY SHINE FANCY, HOLY, Moonlight, Linea Three/Five): XS, S, M, L, XL. Default: M.
- SILK bracelets (CUBIX, SHAPY SPARKLE collections, Sienna One–Five, Za-Ha, Flower Heart, Flower Marquise, Riviera Four/Eight): S/M, L/XL. Default: S/M.
...
export function buildPricesBlock(pricelistYear) {
...
export function buildSystemPrompt(pricelistYear = DEFAULT_PRICELIST) {
  const yr = resolvePricelist(pricelistYear);
  return `${HEADER}
...
// Backwards-compat constant — defaults to the current pricelist.
// Existing callers that haven't been migrated to buildSystemPrompt(year)
// keep working and quote 2026 numbers.
