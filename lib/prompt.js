...
7. **Closure (nylon bracelets — CUTY, MATCHY FANCY, HOLY)** — these nylon-thread bracelets ship with a thread closure that must be either Braided or Non-braided. If the user didn't specify, you MUST ask. Choices: ["Braided", "Non-braided"]. The closureType field on the line must be "braided" or "nonBraided". Note: a Non-braided closure only comes in the grouped sizes S/M and L/XL (Braided keeps the full XS–XL range). CUBIX is non-braided-only — never ask, always emit closureType "nonBraided" (sizes S/M, L/XL). MULTI THREE, MULTI FOUR, MULTI FIVE, and SHAPY SHINE FANCY are braided-only — never ask, always emit closureType "braided".${rivieraClosureNote} Silk bracelets (Shapy Sparkle) and necklaces have NO closure.
...
- closureType ("braided" or "nonBraided") for the nylon bracelets (CUTY, MATCHY FANCY, HOLY) — must be specified by user, never assume. CUBIX is always "nonBraided" (never ask). MULTI THREE, MULTI FOUR, MULTI FIVE, and SHAPY SHINE FANCY${canSeeRiviera(access) ? ' and Riviera Four/Eight are' : ' are'} always "braided" (never ask).
