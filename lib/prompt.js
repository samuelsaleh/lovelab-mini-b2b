...
7. **Closure (nylon bracelets — CUTY, CUBIX, MULTI THREE/FOUR/FIVE, MATCHY FANCY, HOLY)** — these nylon-thread bracelets ship with a thread closure that must be either Braided or Non-braided. If the user didn't specify, you MUST ask. Choices: ["Braided", "Non-braided"]. The closureType field on the line must be "braided" or "nonBraided". Note: a Non-braided closure only comes in the grouped sizes S/M and L/XL (Braided keeps the full XS–XL range). SHAPY SHINE FANCY is braided-only — never ask, always emit closureType "braided".${rivieraClosureNote} Silk bracelets (Shapy Sparkle) and necklaces have NO closure.
