...
import { COLLECTIONS, ADMIN_ONLY_COLLECTION_IDS, DEFAULT_PRICELIST, NEW_ORDER_PRICELIST, resolvePricelist, getPrice, getRetail } from './catalog.js';

// Product-name whitelists used in the "PRODUCT NAMES" rule. The new (preview)
// collections are only revealed to admins.
const PRODUCT_NAMES_LEGACY = 'CUTY, CUBIX, MULTI THREE, MULTI FOUR, MULTI FIVE, MATCHY FANCY, SHAPY SHINE FANCY, SHAPY SPARKLE FANCY, SHAPY SPARKLE D VVS, HOLY (D VVS), CUTY NECKLACE, MULTI THREE NECKLACE, MULTI FOUR NECKLACE, MULTI FIVE NECKLACE, CUBIX NECKLACE, MATCHY FANCY NECKLACE, SHAPY SPARKLE NECKLACE, HOLY NECKLACE';
...
// Riviera Four / Eight are braided-only (Sam, Sep 2026). Only mention them to
// callers who can actually see Riviera — the name must not leak into the
// non-admin prompt.
const canSeeRiviera = (access) =>
  !!(access.allowedIds?.has?.('RIV4') || access.allowedIds?.has?.('RIV8'));
...
const buildHeader = (access) => {
  ...
  const rivieraClosureNote = canSeeRiviera(access)
    ? ' Riviera Four and Riviera Eight are braided-only too — never ask, always emit closureType "braided"; their sizes are S, M, L (never S/M or L/XL).'
    : '';
...
7. **Closure (nylon bracelets — CUTY, CUBIX, MULTI FOUR/FIVE, MATCHY FANCY, HOLY)** — these nylon-thread bracelets ship with a thread closure that must be either Braided or Non-braided. If the user didn't specify, you MUST ask. Choices: ["Braided", "Non-braided"]. The closureType field on the line must be "braided" or "nonBraided". Note: a Non-braided closure only comes in the grouped sizes S/M and L/XL (Braided keeps the full XS–XL range). MULTI THREE and SHAPY SHINE FANCY are braided-only — never ask, always emit closureType "braided".${rivieraClosureNote} Silk bracelets (Shapy Sparkle) and necklaces have NO closure.
...
- MATCHY FANCY: Bezel (WW, YY, PP, WY, WP, YP) OR Prong (White, Yellow, Pink)
- SHAPY SHINE FANCY / NECKLACE: Yellow and White only — there is NO Pink housing. At 0.10ct only Bezel (whatever the shape). At 0.30ct+ both Bezel and Prong available
- HOLY: Yellow, White, Pink
- SHAPY SPARKLE FANCY: no housing options (Prong only)
- SHAPY SPARKLE D VVS: Prong or Bezel (required)${moonlightSiennaHousing}${iconixHousing}
...
- SHAPY SHINE FANCY / NECKLACE: Heart, Pear, Marquise, Oval, Emerald, Cushion. At 0.10ct only Heart, Pear, Marquise, Oval, Emerald (no Cushion).
- SHAPY SPARKLE FANCY: Round, Pear, Oval, Heart, Princess, Cushion, Marquise, Emerald, Long Cushion
- SHAPY SPARKLE D VVS: Round, Pear, Oval, Heart, Princess, Cushion, Marquise, Emerald (no Long Cushion)
...
- closureType ("braided" or "nonBraided") for the nylon bracelets (CUTY, CUBIX, MULTI THREE/FOUR/FIVE, MATCHY FANCY, HOLY) — must be specified by user, never assume. SHAPY SHINE FANCY${canSeeRiviera(access) ? ' and Riviera Four/Eight are' : ' is'} always "braided" (never ask).
...
const PROMPT_LINES = [
  ...
  { id: 'SSRD',  label: 'SHAPY SPARKLE D VVS (In-house, 0.50–0.70)', cert: 'inhouse', skipNulls: true },
  { id: 'SSRD',  label: 'SHAPY SPARKLE D VVS (IGI, 1.00)', cert: 'igi', skipNulls: true },
...
// Backwards-compat constant — the list a new order starts on (admin view,
// i.e. includes the preview collections). Existing callers that haven't been
// migrated to buildSystemPrompt(year, opts) keep working and quote the
// October 2026 numbers.
export const SYSTEM_PROMPT = buildSystemPrompt(NEW_ORDER_PRICELIST);
