...
export async function canAccessOrganization(supabase, user, profile, organizationId) {
  ...
  // The caller's session has already been authenticated. Membership lookup
  // must use the server-only admin client because organization_memberships has
  // self-referential RLS; querying it with the user client can produce a false
  // "not a member" result for a legitimate member.
