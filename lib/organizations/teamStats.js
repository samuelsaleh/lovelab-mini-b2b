...
export function aggregateTeamStats({
  ...
  for (const doc of documents) {
    ...
    // A document can match both the member-creator and organization-event
    // branches of the team query. Count its immutable ID once even if a future
    // query/join returns the row through both branches.
    if (doc.id && countedDocumentIds.has(doc.id)) continue;
    if (doc.id) countedDocumentIds.add(doc.id);
