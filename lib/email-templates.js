...
export function welcomeEmployeeWithPasswordEmail(employeeName, email, tempPassword, signInUrl, siteUrl) {
  return {
    ...
    html: layout(siteUrl, `
      <h2 style="color: #1a1a1a; margin: 0 0 8px;">Welcome, ${employeeName}!</h2>
      <p style="color: #555; font-size: 15px; margin: 0 0 16px;">
...
export function upgradeEmployeeEmail(employeeName, siteUrl) {
  return {
    ...
    html: layout(siteUrl, `
      <h2 style="color: #1a1a1a; margin: 0 0 8px;">Hi ${employeeName},</h2>
      <p style="color: #555; font-size: 15px; margin: 0 0 24px;">
...
export function orderEventEmail({
  ...
  if (kind === 'sent_to_client') {
    ...
    actorLabel = 'Sent by';
    extraRows = `<tr><td style="padding:6px 0;color:#666">Sent to</td><td style="padding:6px 0;font-weight:600">${recipient || '—'}</td></tr>`;
  } else if (kind === 'updated') {
    subject = isOrder ? `Order updated: ${who} — €${amount}` : `Quote updated: ${who}`;
    heading = `${Noun} Updated`;
    actorLabel = 'Updated by';
    extraRows = replaced
      ? `<tr><td style="padding:6px 0;color:#666">Replaces</td><td style="padding:6px 0">the version dated ${replaced} (now in Trash)</td></tr>`
      : '';
  } else {
  ...
  return {
    subject,
    html: layout(siteUrl, `
      <h2 style="color: ${BRAND_COLOR}; margin: 0 0 16px;">${heading}</h2>
      <table style="width:100%;border-collapse:collapse;font-size:14px">
        <tr><td style="padding:6px 0;color:#666;width:140px">Client</td><td style="padding:6px 0;font-weight:600">${who}</td></tr>
        ...
        ${extraRows}
        <tr><td style="padding:6px 0;color:#666">${actorLabel}</td><td style="padding:6px 0">${actorName || '—'}</td></tr>
        <tr><td style="padding:6px 0;color:#666">Type</td><td style="padding:6px 0;text-transform:capitalize">${noun}</td></tr>
      </table>
