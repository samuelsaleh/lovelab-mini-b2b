...
// public/catalogues/email holds 300 dpi re-exports built by
// scripts/build-email-catalogues.mjs; every one of them lands under 9 MB.
...
export async function readOrderEmailCatalogue(lang, { reservedBytes = 0 } = {}) {
  const tryRead = async (filename) => {
    ...
    const emailPath = path.join(process.cwd(), 'public', 'catalogues', 'email', filename);
    const printPath = path.join(process.cwd(), 'public', 'catalogues', filename);

    // The 300 dpi email re-export first; the print master only as a fallback
    // for a catalogue nobody has re-exported yet (it will usually be too big,
    // and the size check below will reject it).
    for (const fullPath of [emailPath, printPath]) {
      let size;
      try {
