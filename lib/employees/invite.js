...
import { findAuthUserByEmail } from '../auth/findAuthUser.js';
import { welcomeEmployeeWithPasswordEmail, upgradeEmployeeEmail } from '../email-templates.js';

export const EMPLOYEE_SELECT = 'id, email, full_name, avatar_url, role, is_agent, is_assistant, has_password_set, created_at, agent_status, commission_rate, agent_since';
...
async function rotateTempPassword(adminSupabase, d, { userId, fullName, existingMetadata }) {
  ...
  // A temporary password mailed to the address is its confirmation: an auth
  // user left unconfirmed by an unclicked magic link would otherwise be
  // refused with "Email not confirmed" the moment they type it.
...
export async function inviteEmployee(adminSupabase, options, deps = {}) {
  ...
  if (existingProfile) {
    ...
    let authUser = null;
    authUser = await findAuthUserByEmail(adminSupabase, emailLower);
