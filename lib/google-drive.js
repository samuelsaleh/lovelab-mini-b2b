...
/**
 * True when either credential set is present. The backup route asks this
 * before doing anything, so a server that simply has no Drive credentials
 * (the move off Vercel, Sep 2026) logs once instead of emailing an alert
 * every night.
 */
export function hasDriveCredentials() {
  const oauth = process.env.GOOGLE_DRIVE_REFRESH_TOKEN && process.env.GOOGLE_OAUTH_CLIENT_ID && process.env.GOOGLE_OAUTH_CLIENT_SECRET;
  return Boolean(oauth || process.env.GOOGLE_SERVICE_ACCOUNT_KEY);
}
...
/**
 * Best-effort delete of a Drive file by ID. Used when a Fair Assistant
 * image row is removed before n8n finishes processing it — we want the
 * upload cleaned up from Drive too, but a 404 (already moved by n8n) is
 * fine. Caller should not throw on failure; just log it.
 */
export async function deleteFileFromDrive(fileId) {
  ...
  return { deleted: false, status: res.status, error: text.slice(0, 200) };
}
