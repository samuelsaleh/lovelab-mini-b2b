/**
 * Housing colours: pieces sold per colour (analytics).
 *
 * Sam, 23 Sep 2026. The housing colour of an order row is `row.bpColor`.
 * Stored values are messy ("White", "White Gold", "Bezel White",
 * "Prongs white", "WW", "WWW", "YWP", "Yellow + Yellow", the five mattes,
 * junk) and are normalised here into fixed buckets. A piece counts once in
 * its bucket, qty-weighted; a two- or three-metal piece is its own "mixed"
 * bucket rather than being split into housings.
 *
 * Availability was shown briefly (internal orders − sales) and dropped the
 * same day at Sam's request: only Sold is displayed.
 *
