...
export async function generatePDF(element, filename, options = {}) {
  ...
  if (mobile && pageElements.length > 0) {
    ...
    // Force desktop width so the PDF looks correct (not squeezed to phone width)
    ...
    for (let i = 0; i < pageElements.length; i++) {
      ...
      // Also fix inner table wrapper (the div with overflowX: auto)
      ...
      // Also force the inner table to not use minWidth (it should fit in 1020px)
      ...
      const canvas = await captureElement(pageEl, scale, quality, DESKTOP_WIDTH);
