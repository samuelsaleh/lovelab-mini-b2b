...
async function capturePerPage(pdf, pageElements, scale, quality, pdfWidth, pageHeight) {
  ...
  for (let i = 0; i < pageElements.length; i++) {
    ...
    if (drawH <= pageHeight) {
      ...
      // Page content is taller than one PDF page -- split into multiple PDF pages
      // instead of shrinking everything down.
      const pageCanvasPx = (pageHeight / drawH) * canvas.height;
      let yPos = 0;
      let first = true;
      ...
      while (yPos < canvas.height) {
        ...
        const sliceH = Math.min(pageCanvasPx, canvas.height - yPos);
        ...
        const sliceDrawH = (sliceH / canvas.width) * pdfWidth;
        pdf.addImage(sliceImg, 'JPEG', 10, 10, drawW, sliceDrawH, undefined, 'MEDIUM');
