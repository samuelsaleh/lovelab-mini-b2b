...
export async function generatePDF(element, filename, options = {}) {
  ...
  // Ensure any parent scroll containers are scrolled to top before capture.
  // The OrderForm content lives inside #order-form-scroll-area, not the window,
  // so window.scrollY is meaningless and can cause blank captures on mobile.
  const scrollParent = element.closest('#order-form-scroll-area') || element.parentElement;
  if (scrollParent) {
    scrollParent.scrollTop = 0;
  }

  // Use a lower scale on mobile to avoid memory issues and blank canvases
  const isMobileDevice = typeof window !== 'undefined' && window.innerWidth < 768;
  const captureScale = isMobileDevice ? Math.min(scale, 1.5) : scale;
