...
async function captureElement(element, scale, quality, forceWidth) {
  ...
  // imageTimeout caps how long html2canvas waits for any single image to
  // load before giving up on it — without it, a stalled image in Safari's
  // cloned DOM can hang the whole capture indefinitely (the root cause of
  // the "order spins forever, never saves" bug on Mac Safari).
...
async function capturePerPage(pdf, pageElements, scale, quality, pdfWidth, pageHeight) {
  ...
  // Must match the .order-form-page maxWidth in OrderForm.jsx.
  ...
  for (let i = 0; i < pageElements.length; i++) {
    ...
    const drawW = pdfWidth;
    const drawH = pdfWidth / imgAspect;

    if (drawH <= pageHeight) {
      pdf.addImage(imgData, 'JPEG', 10, 10, drawW, drawH, undefined, 'MEDIUM');
    } else {
      // The rendered logical page is taller than one A4 PDF page. We used
      // to slice it into multiple PDF pages, which silently broke the
      // "Page X of Y" footer (the X/Y reflects logical pages, but the PDF
      // ended up with extra unlabeled middle pages — the Oxygene 3-page
      // bug 2026-05-12). Shrink-to-fit instead: scale the whole page down
      // proportionally so the PDF always has exactly one PDF page per
      // logical page. The row-count caps in OrderForm.jsx keep this from
      // being needed in practice; this branch is the regression backstop.
      console.warn(
        `[pdf] page ${i + 1} overflowed (drawH=${drawH.toFixed(1)}mm vs ${pageHeight}mm). ` +
        `Shrink-to-fit applied. Consider lowering PRINT_ROWS_PER_PAGE.`,
      );
      const fitW = pageHeight * imgAspect;
      pdf.addImage(imgData, 'JPEG', 10, 10, fitW, pageHeight, undefined, 'MEDIUM');
    }
  }
}
...
export async function generatePDF(element, filename, options = {}) {
  const {
    ...
    format = 'a4',
    // scale 2.0 (was 1.6 pre-2026-05-12): the 1.6× capture left tight
    // numeric columns blurry in print. 2.0× costs ~30% more file size on
    // an A4 landscape with JPEG/MEDIUM compression which is well within
    // Resend's 40 MB attachment cap, and produces visibly sharper edges.
    scale = 2.0,
    quality = 0.92,
  } = options;
