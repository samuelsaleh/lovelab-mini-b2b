...
async function capturePerPage(pdf, pageElements, scale, quality, pdfWidth, pageHeight) {
  ...
  for (let i = 0; i < pageElements.length; i++) {
    ...
    if (pdfH <= pageHeight) {
      ...
      pdf.addImage(imgData, 'JPEG', 10, 10, pdfWidth, pdfH);
    } else {
      // Content exceeds page height -- scale proportionally to fit
      const scaleFactor = pageHeight / pdfH;
      const scaledWidth = pdfWidth * scaleFactor;
      const xOffset = 10 + (pdfWidth - scaledWidth) / 2; // Center horizontally
      pdf.addImage(imgData, 'JPEG', xOffset, 10, scaledWidth, pageHeight);
    }
  }
}
