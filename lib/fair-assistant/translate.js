...
- "signoff" is a closing phrase followed by a person's name and company on their own lines. Translate the closing phrase ("Warm regards" -> "Herzliche Grüße", "Cordiali saluti", ...) and keep the name, company and line breaks exactly as given. Never leave the closing phrase in English.
...
export async function translateEmailSlots(slots, languageCode) {
  ...
  // (lead_type, language-set), so one call covers a whole batch.
  let text;
  try {
  ...
  try {
    ...
    // 'subject' MUST be here. It was omitted originally, so the translated
    // subject was thrown away and every lead — Italian, German, Japanese —
    // got an English subject line on top of a translated body.
...
export function buildEmailForLead({
  ...
  if (blocks.length > 1) {
    // The shell already renders the PRIMARY language's greeting and paragraph1
    // above this slot. Repeating every block here (primary included) printed
    // the French greeting and opening paragraph twice to every Belgian lead.
  ...
  const html = renderFairOutreachEmail({
    ...
    // primary language too — see chrome-strings.js.
    lang: primary.lang,
  });
