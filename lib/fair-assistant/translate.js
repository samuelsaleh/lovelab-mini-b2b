...
- The JSON must have these keys: "headline", "paragraph1", "paragraph2", "signoff", "ctaLine". Include all five keys even if a value is an empty string.
...
export async function translateEmailSlots(slots, languageCode) {
  ...
  try {
    ...
    const required = ['headline', 'paragraph1', 'paragraph2', 'signoff', 'ctaLine'];
    const merged = { ...slots };
...
export function buildEmailForLead({
  ...
  // Explicit subject from the form wins. Personalize it with the same
  // template slots so "Following up from {fairName}" expands properly.
