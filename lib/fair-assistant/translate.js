...
/**
 * Why the model call failed, in words an admin can act on. A rejected or
 * missing key is a server problem, not a translation problem (Sam, 15 Sep
 * 2026: "API key is invalid" read as if the translation had gone wrong).
 */
export function describeApiFailure(err) {
  const msg = String(err?.message || err || '');
  if (/api key|authentication|not configured|x-api-key/i.test(msg)) {
    return "the server's AI key (ANTHROPIC_API_KEY) is missing or invalid — ask the server admin to set it";
  }
  return `the reply could not be read (${msg})`;
}
...
export async function verifyLanguage({ createMessage, label, languageCode, translated }) {
  try {
    const res = await createMessage({
