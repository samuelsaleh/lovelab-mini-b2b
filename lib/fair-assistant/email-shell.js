...
import { chromeStrings, localizeButtonLabel } from '@/lib/fair-assistant/chrome-strings';
...
const DEFAULT_BUTTON1 = { label: 'Visit Our Website', url: 'https://lovelab-antwerp.com/' };
const DEFAULT_BUTTON2 = { label: 'B2B Login',         url: 'https://app.lovelab-antwerp.com/' };
...
function contactCardHtml(c) {
  ...
  // Pixel widths on purpose. iOS Gmail rescales the 600px email to the screen
  // and resolves percentage widths on nested tables against the SCALED
  // viewport, not the container — a 100%-in-100% card came out at ~40% and
  // sat against the left edge. 540 = 600 minus the 30px side padding.
  return `
    <table role="presentation" width="540" align="center" cellpadding="0" cellspacing="0" border="0" style="width:540px;max-width:100%;margin:0 auto;table-layout:fixed;border-collapse:separate;background-color:#ffffff;border:1px solid ${BRAND.border};border-radius:12px;">
      <tr>
        <td align="center" style="padding:24px 30px;text-align:center;">
          <p style="margin:0 0 14px 0;font-size:11px;color:${BRAND.gold};text-transform:uppercase;letter-spacing:0.2em;font-weight:700;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">${esc(c.contact)}</p>
          ${line('hello@love-lab.com')}
          ${line('+32 475 32 10 32')}
          ${line('lovelab-antwerp.com')}
...
export function renderFairOutreachEmail({
  ...
  const raw1 = (button1?.url || button1?.label) ? button1 : DEFAULT_BUTTON1;
  const raw2 = (button2?.url || button2?.label) ? button2 : DEFAULT_BUTTON2;
  const b1 = { ...raw1, label: localizeButtonLabel(raw1.label, lang) };
  const b2 = { ...raw2, label: localizeButtonLabel(raw2.label, lang) };
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="background-color:#ffffff;max-width:600px;">
          ...
          <tr>
            <td align="center" style="padding:32px 50px 8px 50px;">
              <h2 style="margin:0;font-family:Georgia,'Times New Roman',serif;font-size:22px;font-weight:400;color:${BRAND.plum};letter-spacing:0.05em;text-transform:uppercase;">${esc(c.explore)}</h2>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td align="center" style="padding:0 30px 16px 30px;">
              <table role="presentation" width="540" align="center" cellpadding="0" cellspacing="0" border="0" style="width:540px;max-width:100%;margin:0 auto;table-layout:fixed;border-collapse:collapse;">
                <tr>
                  <td align="center" width="270" style="width:50%;padding:10px;">
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid ${BRAND.border};border-radius:20px;width:100%;">
                      <tr><td align="center" style="padding:10px 5px;"><p style="margin:0;font-size:10px;color:${BRAND.textLight};">${esc(c.returns)}</p></td></tr>
                    </table>
                  </td>
                  <td align="center" width="270" style="width:50%;padding:10px;">
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid ${BRAND.border};border-radius:20px;width:100%;">
                      <tr><td align="center" style="padding:10px 5px;"><p style="margin:0;font-size:10px;color:${BRAND.textLight};">${esc(c.shipping)}</p></td></tr>
                    </table>
                  </td>
                </tr>
              </table>
          ...
          <tr>
            <td align="center" style="padding:8px 30px 28px 30px;">
              ${contactCardHtml(c)}
            </td>
          </tr>
          <tr>
...
function absUrl(siteUrl, path) {
  ...
  const base = (siteUrl || 'https://app.lovelab-antwerp.com').replace(/\/$/, '');
