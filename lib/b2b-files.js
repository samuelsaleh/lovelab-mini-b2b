...
export const BRAND_DOCUMENT_FILES = [
  ...
  {
    name: 'LoveLab Brand Presentation — German.pdf',
    path: '/BRAND PRESENTATION DOCS/LoveLab_Brand_Presentation_General_DE.pdf',
  },
  {
    name: 'LoveLab Brand Presentation — Italian.pdf',
    path: '/BRAND PRESENTATION DOCS/LoveLab_Brand_Presentation_General_IT.pdf',
  },
  {
...
/**
 * What every agent may download and email: catalogues and brand documents.
 * Price lists, EAN codes and order packs stay admin-only (Sam, 22 Sep 2026).
 */
export const AGENT_RESOURCE_FILES = [...CATALOGUE_FILES, ...BRAND_DOCUMENT_FILES];

export function isAgentResourcePath(path) {
  return AGENT_RESOURCE_FILES.some((f) => f.path === path);
}
