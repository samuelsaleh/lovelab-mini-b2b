...
export async function lookupCompany(companyName, country) {
  ...
  const prompt = `Find the business address and EU VAT identification number for the company "${companyName}" in ${country}.

Search strategy:
1. Search for the company's official website
2. On the website, look for the "${legalPageHint}" page (usually linked in the footer) — this page typically contains the VAT number, company registration, and full registered address
3. Also check: footer of every page, "about us", "contact", CGV/terms, privacy policy, imprint, and any legal pages
4. If the company has a KvK (Chamber of Commerce) registration in the Netherlands, or similar business registry in other countries, search those registries too
5. Search business directories and public company databases

The VAT number format for ${country} typically starts with the country code (e.g. NL for Netherlands, BE for Belgium, FR for France, DE for Germany, etc.).

Return ONLY a valid JSON object with these exact fields:
{ "address": "street name + number", "city": "city name", "zip": "postal code", "country": "${country}", "vat": "full VAT number including country prefix" }

If you cannot find a specific field, use an empty string "". Output ONLY the JSON object, no other text.`
  ...
  console.log('[lookupCompany] Searching for:', companyName, 'in', country)
  ...
  console.log('[lookupCompany] Raw response:', raw)
  
  const parsed = extractJSON(raw)
  console.log('[lookupCompany] Parsed:', parsed)
  ...
  console.log('[lookupCompany] Result:', result)
  return result
}
