...
// ────────────────────────────────────────────────────────────────────────
// Data shaping (pure, testable) — unchanged from previous version
// ────────────────────────────────────────────────────────────────────────
export function buildReportData({
  ...
  // We filter by the ORDER's creation date (when the sale happened), not
  // by when the "customer paid?" checkbox was ticked. This way April's report
  // shows orders from April regardless of whether the admin ticked them today
  // or last week. A commission is included when:
  //   1. It has customer_paid_at set (customer has paid the invoice)
  //   2. The underlying order was created in the report period
  //      (falls back to commission created_at for bonuses with no document)
  const eligible = (commissions || []).filter((c) => {
    if (!c) return false;
    if (c.status === 'cancelled' || c.status === 'paid') return false;
    if (!c.customer_paid_at) return false;
    const orderDate = c.document?.created_at || c.created_at;
    if (!inWindow(orderDate)) return false;
    return ['order', 'new_client_bonus', 'bonus'].includes(c.type);
  });
...
export async function generateCommissionReport({ data, logoBuffer = null, demoMode = false }) {
  ...
  // ════════════════════════════════════════════════════════════════════
  r = renderSectionHeader(ws, r, 'COMMISSIONS THIS MONTH', PLUM, PLUM_TINT);
