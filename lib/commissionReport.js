 ...
 * @param {Array<{id: string, name: string}>} [args.members] - Organization
 *   mode (Phase 31): the full member roster. When provided, each eligible
 *   commission (which must then carry `agent_id`) is also bucketed per
 *   member into `memberBreakdown`, so the org report can show what each
 *   sub-agent contributed while the grand total stays one number.
 * @param {{id: string, name: string}} [args.organization] - Organization
 *   mode metadata. Stored in the snapshot and used by the xlsx renderer to
 *   relabel "TOTAL DUE TO AGENT" as "TOTAL DUE TO ORGANIZATION".
 */
export function buildReportData({
  ...
  const eligible = (commissions || []).filter((c) => {
    ...
    // Phase 29: a row already pulled into a prior report ("Reported") must not
    // be swept into another one — it's waiting for its payment to be recorded.
    if (c.report_id) return false;
  ...
  // Organization mode: pre-seed one bucket per member so agents with zero
  // activity still show up (with zeros) in the breakdown.
  const breakdownById = Array.isArray(members) && members.length > 0 ? new Map() : null;
  if (breakdownById) {
    for (const m of members) {
      if (!m?.id) continue;
      breakdownById.set(m.id, {
        agent_id: m.id,
        name: m.name || 'Agent',
        orderCount: 0,
        commissionTotal: 0,
  ...
  const bumpBreakdown = (agentId, { commission = 0, bonus = 0, isOrder = false } = {}) => {
    if (!breakdownById || !agentId) return;
    if (!breakdownById.has(agentId)) {
      breakdownById.set(agentId, {
        agent_id: agentId, name: 'Agent', orderCount: 0, commissionTotal: 0, bonusTotal: 0, total: 0,
      });
    }
    const b = breakdownById.get(agentId);
    if (isOrder) b.orderCount += 1;
    b.commissionTotal = round2(b.commissionTotal + commission);
    b.bonusTotal = round2(b.bonusTotal + bonus);
    b.total = round2(b.commissionTotal + b.bonusTotal);
  };
  ...
  for (const c of eligible) {
    ...
    const clientName =
      doc.client_company ||
      doc.client_name ||
      // Phase 27: manual "quick orders" have no document — fall back to the
      // client_label the admin typed when recording the order.
      c.client_label ||
    ...
    if (c.type === 'order') {
      ...
      bumpBreakdown(c.agent_id, { commission: round2(commission), isOrder: true });
    } else {
      bonusesArr.push({
        date: dateIso,
        client: clientName,
      ...
      bumpBreakdown(c.agent_id, { bonus: round2(commission) });
    }
  }
  ...
  const memberBreakdown = breakdownById
    ? [...breakdownById.values()].sort((a, b) => b.total - a.total || a.name.localeCompare(b.name))
    : null;
  ...
  return {
    ...
    // Organization mode only (Phase 31) — null/absent for classic per-agent
    // reports so old snapshots and their consumers are unaffected.
...
export async function generateCommissionReport({ data, logoBuffer = null, demoMode = false }) {
  // Organization reports (Phase 31) owe one number to the partner company,
  // not to an individual — relabel the due bands accordingly.
  const dueLabel = data.organization ? 'TOTAL DUE TO ORGANIZATION' : 'TOTAL DUE TO AGENT';
  ...
  // ════════════════════════════════════════════════════════════════════
  // TEAM BREAKDOWN — organization reports only (Phase 31). One row per
  // sub-agent so mom sees who contributed what, while payment stays one
  // number to the organization.
  // ════════════════════════════════════════════════════════════════════
  if (Array.isArray(data.memberBreakdown) && data.memberBreakdown.length > 1) {
    r = renderSectionHeader(ws, r, 'TEAM BREAKDOWN (per agent)', PLUM, PLUM_TINT);

    ws.getRow(r).height = 22;
    ws.mergeCells(`A${r}:B${r}`);
    ws.getCell(`A${r}`).value = 'Agent';
    ws.getCell(`C${r}`).value = 'Orders';
    ws.getCell(`D${r}`).value = 'Commission';
    ws.getCell(`E${r}`).value = 'Total due';
    ['A', 'C', 'D', 'E'].forEach((col) => {
      const cell = ws.getCell(`${col}${r}`);
      cell.font = { name: 'Calibri', size: 9, bold: true, color: { argb: TEXT_LABEL } };
      cell.alignment = { horizontal: col === 'A' ? 'left' : 'right', vertical: 'middle', indent: col === 'A' ? 2 : 1 };
      cell.border = bottomOnly(SOFT_BORDER, 'thin');
    });
    r += 1;

    data.memberBreakdown.forEach((m, i) => {
      const bg = i % 2 === 1 ? ROW_ZEBRA : ROW_PLAIN;
      ws.getRow(r).height = 22;
      ws.mergeCells(`A${r}:B${r}`);
      ws.getCell(`A${r}`).value = m.name;
      ws.getCell(`C${r}`).value = m.orderCount;
      ws.getCell(`D${r}`).value = m.commissionTotal;
      ws.getCell(`E${r}`).value = m.total;
      ['A', 'B', 'C', 'D', 'E', 'F'].forEach((col) => {
        const cell = ws.getCell(`${col}${r}`);
        cell.fill = fill(bg);
        cell.font = { name: 'Calibri', size: 11, color: { argb: TEXT_BODY } };
      ...
      ws.getCell(`A${r}`).font = { name: 'Calibri', size: 11, bold: true, color: { argb: PLUM_DARK } };
      ws.getCell(`E${r}`).font = { name: 'Calibri', size: 11, bold: true, color: { argb: PLUM_DARK } };
      r += 1;
    });
    r += 2;
