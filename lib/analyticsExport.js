...
// "5081 Anif", "80336 München", "DE-80336 Munich", "L-1234 Ville".
const POSTAL_CITY_REGEX = /^(?:([A-Za-z]{1,2})-)?(\d{4,6})\s+(.{2,})$/;

/**
 * The dedicated postal code / city fields are almost never filled in — in
 * practice people type "80336 München" into the second address line. Pull the
 * two apart so the export has sortable columns, but only when the line
 * unmistakably starts with a postcode; anything else stays untouched rather
 * than being guessed at. The full address column keeps the original text
 * either way, so nothing is lost.
 */
export function derivePostalAndCity(formState = {}) {
  const postalCode = firstOf(formState.postal_code, formState.zipcode);
  const city = firstOf(formState.city);
  if (postalCode && city) return { postalCode, city };

  const match = POSTAL_CITY_REGEX.exec(clean(formState.addressLine2));
  if (!match) return { postalCode, city };

  const [, countryPrefix, digits, rest] = match;
  return {
    postalCode: postalCode || (countryPrefix ? `${countryPrefix.toUpperCase()}-${digits}` : digits),
    city: city || clean(rest),
  };
}
