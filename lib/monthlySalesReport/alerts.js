 ...
 * MONTHLY_SALES_REPORT_ALERT_TO (in Apps Script: ALERT_TO, else the script's
 * owner). Never to the report's other recipients.
 *
 *   level 'error'   — the report was NOT delivered, or only partly
 *   level 'warning' — delivered, but the data behind it needs a look
 *
 * This is the app's own alarm. If the app or the server is down entirely it
 * can't send anything — then the report simply doesn't arrive on the 1st.
 */
 ...
 */
export async function sendReportAlert({ month, level, problems, delivered, send, recipients = reportRecipients(process.env.MONTHLY_SALES_REPORT_ALERT_TO) }) {
