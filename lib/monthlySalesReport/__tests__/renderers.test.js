...
import { renderReportPdf, reportPdfText, parseSvg, base64ToBytes } from '../pdfDirect.js'
...
describe('renderReportPdf — drawn directly, no browser', () => {
  it('is one valid PDF page, 120 mm wide, as tall as its content', async () => {
    const bytes = await renderReportPdf(buildReportData(loadSample(), '2026-08'))
    expect(String.fromCharCode(...bytes.slice(0, 5))).toBe('%PDF-')
    const doc = await PDFDocument.load(bytes)
    expect(doc.getPageCount()).toBe(1)
    const { width, height } = doc.getPage(0).getSize()
    expect(Math.round(width)).toBe(340) // 120 mm
    expect(height).toBeGreaterThan(900)
  })

  it('has every section, and stamps sample data', async () => {
    const text = await reportPdfText(buildReportData(loadSample(), '2026-08'))
    for (const t of ['August 2026 — sales report', 'SALES BY MONTH', 'SALES BY AGENT · AUGUST', 'B2B VS B2C', 'AGENTS · AUGUST 2026', 'FAIRS · AUGUST 2026', 'ALL FAIRS · 2026 TO DATE']) expect(text).toContain(t)
    expect(text).toContain('SAMPLE DATA — NOT REAL')
  })

  it('carries no sample stamp for real data, says when there were no fairs, and keeps accents', async () => {
    const text = await reportPdfText(real({ agents: [{ id: 'a', name: 'Piotr Kiciński' }] }))
    expect(text).not.toContain('SAMPLE DATA')
    expect(text).toContain('No fairs this month.')
    expect(text).toContain('Piotr Kiciński')
  })
})

describe('pdf helpers', () => {
  it('decodes base64 without atob or Buffer', () => {
    expect(Array.from(base64ToBytes('JVBERi0='))).toEqual([37, 80, 68, 70, 45])
    expect(String.fromCharCode(...base64ToBytes('TG92ZUxhYg=='))).toBe('LoveLab')
  })

  it('reads the chart shapes back from the SVG, unescaping text', () => {
    const svg = parseSvg('<svg width="300" height="100"><rect x="1" y="2" width="3" height="4" fill="#fff"/><path d="M0,0H1Z" fill="#000"/><text x="5" y="6" font-size="7">A &amp; B</text></svg>')
    expect(svg.width).toBe(300)
    expect(svg.elements.map((e) => e.tag)).toEqual(['rect', 'path', 'text'])
    expect(svg.elements[2].text).toBe('A & B')
  })
})
...
describe('svgCharts', () => {
  ...
  it('shortens a long agent name to fit its gutter instead of running off the edge', () => {
    expect(fitLabel('Anna Vandermeulen', 102, 7.5)).toBe('Anna Vandermeulen')
    const cut = fitLabel('SAMPLE WHOLESALE AGENCY FRANCE', 102, 7.5)
    expect(cut.startsWith('SAMPLE')).toBe(true)
...
describe('commission is labelled as a cost to LoveLab', () => {
  ...
  it('shows commission with a minus sign and says who pays whom, in the PDF', async () => {
    const text = await reportPdfText(withCommission())
    expect(text).toContain('−€150')
    expect(text).toMatch(/OWED TO\nAGENT/)
    expect(text).toContain('LoveLab keeps')
    expect(text).toContain('Commission owed to agents')
    expect(text).not.toMatch(/Commission earned|Net of commission|Paid out/)
  })
...
describe('fairs are shown separately, never added together', () => {
  ...
  it('PDF: each fair has its own revenue, no combined total, and says when the fair was in an earlier month', async () => {
    const text = await reportPdfText(twoFairs())
    expect(text).toContain('€31,516')
    expect(text).toContain('€4,404')
    expect(text).not.toContain('€35,920') // the two added together
    expect(text).not.toMatch(/All 2 fairs/)
    expect(text.replace(/\n/g, ' ')).toMatch(/This fair was held 25–27 Jul 2026.*total so far is €32,404 from 2 orders/)
  })
...
describe('sales by month shows the figures', () => {
  it('puts every month total on the chart and a B2B / B2C / total row per month under it', async () => {
    ...
    const text = await reportPdfText(r)
    expect(text).toMatch(/^€113k$/m) // July total on its column, not only the report month
    expect(text).toMatch(/Jul 2026\n€102,000\n€10,806\n€112,806/)
