...
import { markCommissionsPaidOut } from './commissionPaidOut.js';
...
export async function generateAgentReport({
  ...
  const commissionSelect = (labelCol) => `
      id, type, status, commission_rate, commission_amount, order_total,
      created_at, customer_paid_at, document_id${labelCol ? ', client_label' : ''},
      document:documents (
        id, client_name, client_company, total_amount, order_channel, metadata, created_at
      )
    `;
  const loadCommissions = (labelCol) => supabase
    .from('agent_commissions')
    .select(commissionSelect(labelCol))
    .eq('agent_id', agentId)
    .neq('status', 'cancelled')
    .order('created_at', { ascending: true });

  let { data: commissions, error: cErr } = await loadCommissions(true);
  if (cErr && /client_label/.test(cErr.message || '')) {
    ({ data: commissions, error: cErr } = await loadCommissions(false));
  }
  ...
  // ── 9. Mark included commissions paid out ──────────────────────────
  // Mom's workflow: tick Paid? → send report → pay agent. Once the report
  // is generated, those rows must not appear in the next export.
  let paidOut = { marked: 0, ids: [] };
  if (data.includedCommissionIds?.length) {
    paidOut = await markCommissionsPaidOut(supabase, data.includedCommissionIds);
  }
