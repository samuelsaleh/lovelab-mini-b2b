 ...
 *   7. Optionally emails the buffer to dionne@love-lab.com via Resend
 *      when sendEmail=true (manual admin button only — never cron).
 *      Agents are never emailed. Failure logged but does NOT fail the flow.
...
import { linkCommissionsToReport } from './commissionPaidOut.js';
 ...
 * @param {boolean}[args.options.sendEmail=false]
...
export async function generateAgentReport({
  ...
  //      created earlier).
  // `client_label` is added by the Phase 27 migration. Tolerate its absence so
  // report generation keeps working if this code is deployed before the
  // migration has run on a given environment (forward-deploy safety net).
  // `report_id` (Phase 29) flags rows already pulled into a previous report so
  // buildReportData can skip them. Both optional columns degrade gracefully if
  // their migration hasn't run yet on a given environment.
  const commissionSelect = ({ label, report: reportCol }) => `
      id, type, status, commission_rate, commission_amount, order_total,
      created_at, customer_paid_at, document_id${label ? ', client_label' : ''}${reportCol ? ', report_id' : ''},
      document:documents (
        id, client_name, client_company, total_amount, order_channel, metadata, created_at
      )
    `;
  const loadCommissions = (opts) => supabase
    .from('agent_commissions')
    .select(commissionSelect(opts))
    .eq('agent_id', agentId)
    .neq('status', 'cancelled')
    .order('created_at', { ascending: true });

  let { data: commissions, error: cErr } = await loadCommissions({ label: true, report: true });
  if (cErr && /report_id/.test(cErr.message || '')) {
    ({ data: commissions, error: cErr } = await loadCommissions({ label: true, report: false }));
  }
  if (cErr && /client_label/.test(cErr.message || '')) {
    ({ data: commissions, error: cErr } = await loadCommissions({ label: false, report: false }));
  }
  ...
  const insertPayload = {
    ...
    email_recipient: emailRes.sent ? emailRes.recipient : null,
  ...
  // ── 9. Link included commissions to this report ────────────────────
  // Phase 29: mom's workflow is tick Paid? → send report → record payment.
  // Sending the report no longer marks rows paid — it just links them to the
  // report ("Reported" state) so (a) the next export skips them and (b)
  // recording the payment later can settle exactly this set + stamp invoices.
  let linked = { linked: 0, ids: [] };
  if (data.includedCommissionIds?.length) {
    linked = await linkCommissionsToReport(supabase, report.id, data.includedCommissionIds);
  }
...
/**
 * Phase 31 — organization-level report ("one payment per organization").
 *
 * Multi-member partner orgs (e.g. Showroom Accestory) are paid as ONE
 * entity: this sweeps every member's ready commissions into a single
 * report keyed to the org OWNER, with a per-agent breakdown inside so
 * each sub-agent's contribution stays visible. Recording one payment
 * against this report then settles all members' rows at once (the
 * existing report_id settlement path is already agent-agnostic).
 *
 * Single-member orgs (solo agents — everyone historically) delegate to
 * the classic per-agent flow, byte-identical behavior.
 *
 * @param {object} args
 * @param {object} args.supabase        — admin (service-role) Supabase client
 * @param {string} args.organizationId
 * @param {{ start, end, label, key, snapshot? }} args.period
 * @param {object} [args.options]       — same options as generateAgentReport
 */
export async function generateOrganizationReport({
  ...
  // ── 2. Load active memberships (owner + members) ───────────────────
  const { data: memberships, error: memErr } = await supabase
    .from('organization_memberships')
    .select('user_id, role, profiles:user_id(id, full_name, email, agent_status)')
    .eq('organization_id', organizationId)
    .is('deleted_at', null);
  if (memErr) throw new Error(`Failed to load organization members: ${memErr.message}`);

  const members = memberships || [];
  if (members.length === 0) {
    throw new Error('Organization has no active members');
  }

  const ownerMembership = members.find((m) => m.role === 'owner');
  if (!ownerMembership) {
    throw new Error('Organization has no owner — cannot key the report');
  }

  // ── 3. Solo orgs keep the exact per-agent flow (no behavior change) ─
  if (members.length === 1) {
    return generateAgentReport({
      supabase,
      agentId: ownerMembership.user_id,
      period,
      options,
    });
  }

  // ── 4. Load ALL members' commissions (agent_id included so the
  //       breakdown can bucket rows per sub-agent) ─────────────────────
  const memberIds = members.map((m) => m.user_id);
  const orgCommissionSelect = ({ label, report: reportCol }) => `
      id, agent_id, type, status, commission_rate, commission_amount, order_total,
      created_at, customer_paid_at, document_id${label ? ', client_label' : ''}${reportCol ? ', report_id' : ''},
      document:documents (
        id, client_name, client_company, total_amount, order_channel, metadata, created_at
      )
    `;
  const loadOrgCommissions = (opts) => supabase
    .from('agent_commissions')
    .select(orgCommissionSelect(opts))
    .in('agent_id', memberIds)
    .neq('status', 'cancelled')
    .order('created_at', { ascending: true });

  let { data: commissions, error: cErr } = await loadOrgCommissions({ label: true, report: true });
  if (cErr && /report_id/.test(cErr.message || '')) {
    ({ data: commissions, error: cErr } = await loadOrgCommissions({ label: true, report: false }));
  }
  if (cErr && /client_label/.test(cErr.message || '')) {
    ({ data: commissions, error: cErr } = await loadOrgCommissions({ label: false, report: false }));
  }
  if (cErr) throw new Error(`Failed to load commissions: ${cErr.message}`);

  // ── 5. Shape — the "agent" of an org report is the organization
  //       itself, keyed to the owner's user id so the existing
  //       commission_reports schema and payment flow work untouched. ───
  const ownerProfile = ownerMembership.profiles || {};
  const agentForReport = {
    id: ownerMembership.user_id,
    full_name: org.name,
    email: ownerProfile.email || '',
    commission_rate: org.commission_rate ?? 0,
  };
  const rosterForBreakdown = members.map((m) => ({
    id: m.user_id,
  ...
  // ── 6. Render + archive (same pipeline as per-agent reports) ───────
  const logoBuffer = await loadLogo();
  const xlsxBuffer = await generateCommissionReport({ data, logoBuffer });

  const safeName = sanitiseFilenamePart(org.name || organizationId);
  const storagePath = `${safeName}/${safeName} - ${period.key}.xlsx`;
  ...
  if (uploadToDrive) {
    driveRes = await uploadCommissionReportToDrive({
      buffer: xlsxBuffer,
      agentName: org.name || 'Organization',
      periodKey: period.key,
      cachedFolderId: null,
    });
    if (driveRes.error) {
      console.warn(`[commissionReportService] Drive upload failed for org ${organizationId}:`, driveRes.error);
    }
  }
  ...
  if (sendEmail) {
    emailRes = await sendCommissionReportEmail({
      buffer: xlsxBuffer,
      agent: { name: org.name || 'Organization', email: ownerProfile.email || '' },
      period,
      totals: data.totals,
    });
    if (!emailRes.sent) {
      console.warn(`[commissionReportService] Email failed for org ${organizationId}:`, emailRes.reason || emailRes.error);
    }
  }

  // ── 7. Record the report, keyed to the owner ───────────────────────
  const insertPayload = {
    agent_id: ownerMembership.user_id,
    period_start: new Date(period.start).toISOString(),
    period_end: new Date(period.end).toISOString(),
    period_label: period.label,
    period_key: period.key,
  ...
  // ── 8. Link every member's included commissions to this org report ──
  let linked = { linked: 0, ids: [] };
  if (data.includedCommissionIds?.length) {
    linked = await linkCommissionsToReport(supabase, report.id, data.includedCommissionIds);
  }
