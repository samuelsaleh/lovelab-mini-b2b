...
export async function ensureAgentFolderEvent(adminSupabase, userId) {
  ...
  // Solo-org reuse guard (dedup hardening): if this agent's org has exactly one
  // active member (them) and already owns a single agent folder, reuse it even
  // when the folder name no longer matches (e.g. it was renamed). Without this
  // a rename would spawn a second folder for the same person. Multi-member orgs
  // are exempt — each member legitimately gets their own named folder.
  //
  // Positive-confirmation only + fully defensive: any lookup failure (or a
  // lightweight client that doesn't support the chain) leaves isSoloOrg=false
  // so we fall through to the normal create path and never wrongly collapse a
  // multi-member org onto one folder.
  if (targetOrgId) {
    try {
      ...
      const isSoloOrg = memberIds.size === 1;
      if (isSoloOrg) {
        const { data: orgAgentFolders } = await adminSupabase
          .from('events')
          .select('id, name')
          .eq('type', 'agent')
          .eq('organization_id', targetOrgId);
        if ((orgAgentFolders || []).length >= 1) {
          const match = orgAgentFolders.find((f) => nameKey(f.name) === wantName);
          return (match || orgAgentFolders[0]).id;
        }
      }
    } catch (guardErr) {
      console.error('[ensureAgentFolderEvent] solo-org reuse guard skipped:', guardErr.message);
    }
  }
