...
export async function validateVAT(vatString) {
    ...
    try {
      ...
      if (data && typeof data === 'object' && typeof data.result === 'string') {
        ...
        if (isRetryableErrorCode(lastErrorCode) && attempt < maxRetries - 1) {
          ...
          console.debug(`VIES ${lastErrorCode}, retrying in ~${delay}ms (attempt ${attempt + 1}/${maxRetries})`)
          await sleep(jitterDelay(delay))
          continue
        }
      ...
      if (attempt < maxRetries - 1) {
        const delay = baseDelay * Math.pow(2, attempt)
        console.debug(`Network error, retrying in ~${delay}ms (attempt ${attempt + 1}/${maxRetries})`)
        await sleep(jitterDelay(delay))
        continue
      }
      break
  ...
  console.debug(`VIES API failed after ${maxRetries} attempts (${lastErrorCode}), marking as unverified`)
