...
export function publicAssetUrl(origin, path) {
  return `${String(origin || '').replace(/\/$/, '')}${publicAssetHref(path)}`;
}

/** Allowed static resource paths used by /api/resources/send-email. */
export const ALLOWED_RESOURCE_PATH_RE =
  /^\/(Price Lists|Ean Codes|catalogues|BRAND PRESENTATION DOCS|IGI Excel)(\/[^/]+)+\.(xlsx|pdf)$/i;
