...
import { generatePackExcelBuffer } from './packExcel.js'
...
/**
 * List the packs shown in the admin "Packs" folder: every non-private pack
 * (global + seed + restricted). Private agent packs stay private. Restricted
 * packs are included because this folder is admin-only and admins can see all
 * non-private packs (matches the packs RLS rule `is_admin() AND scope <>
 * 'private'`). Ordered seed-first, then chronological — matching GET /api/packs.
 *
 * @param {object} admin — service-role Supabase client
