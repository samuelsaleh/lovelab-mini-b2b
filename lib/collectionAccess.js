...
// October 2026 (`2026-10`) is the current / default list — everyone sees it.
// The older 2025 and 2026 lists stay available only for Alberto and Dionne
// (legacy client transitions). Email is the gate, not admin role: BuilderPage
// spreads role:'admin' for admins, and other admins must not regain 2025/2026.
export const LEGACY_PRICELISTS = new Set(['2025', '2026'])
export const LEGACY_PRICELIST_EMAILS = new Set([
  'alberto@love-lab.com',
  'albertosaleh@gmail.com',
  'dionne@love-lab.com',
])

// Kept as an empty set so older imports that still reference PREVIEW_PRICELISTS
// do not crash; October is no longer a preview list.
export const PREVIEW_PRICELISTS = new Set()

function canSeeLegacyPricelists(profile) {
  if (!profile) return false
  const email = String(profile.email || '').trim().toLowerCase()
  return LEGACY_PRICELIST_EMAILS.has(email)
}

export function canSeePricelist(pricelistYear, profile) {
  const year = String(pricelistYear)
  if (year === '2026-10') return true
  if (LEGACY_PRICELISTS.has(year)) return canSeeLegacyPricelists(profile)
  return true
}

// The price lists to offer as a choice. `activeYear` is always included even
// when the profile could not have chosen it: a document saved on a legacy list
// must keep showing which list priced it, and quietly relabelling it to
// October would print a price list name that does not match the prices.
