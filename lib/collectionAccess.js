...
// Sam, 15 Sep 2026: only the October 2026 list is offered from now on. The
// 2025 and pre-October 2026 lists are retired from the picker but stay in
// PRICELISTS, because an order saved on them must keep re-pricing on them —
// getVisiblePricelists keeps a document's own list visible for that reason.
//
// The October list used to be admin-and-Piotr only (it reprices the preview
// collections, which most agents cannot sell anyway). As the only list left it
// is visible to everyone; the collection gate still decides who sells what.
export const PREVIEW_PRICELISTS = new Set([])
export const RETIRED_PRICELISTS = new Set(['2025', '2026'])
...
// when the profile could not have chosen it, or when the list is retired: a
// document saved on the 2025 list must keep showing which list priced it, and
// quietly relabelling it would print a price list name that does not match
// the prices.
export function getVisiblePricelists(profile, activeYear = null) {
  const active = activeYear == null ? null : String(activeYear)
  return PRICELISTS.filter((year) => year === active || (!RETIRED_PRICELISTS.has(year) && canSeePricelist(year, profile)))
}
