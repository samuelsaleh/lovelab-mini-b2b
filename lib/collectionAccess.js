...
// The October 2026 revision reprices Moonlight / Sienna / Za-Ha (admin/Piotr
// preview) AND Iconix Flower / Riviera / Linea (visible to every agent). So the
// list is no longer admin-only: any profile that can sell a collection with a
// '2026-10' bucket needs to see the list that carries those prices. Deriving
// the audience from the catalog keeps a single place to edit when October
// gains or loses a collection.
export const PREVIEW_PRICELISTS = new Set(['2026-10'])

function collectionsRepricedIn(pricelistYear) {
  const year = String(pricelistYear)
  return COLLECTIONS.filter((c) => c.prices?.[year] || c.retail?.[year])
}
...
export function canSeePricelist(pricelistYear, profile) {
  ...
  const repriced = collectionsRepricedIn(year)
  if (repriced.length === 0) return false
  return repriced.some((c) => canSeeCollection(c.id, profile))
}
