...
import { emailDeliveriesAvailable, emailOpensAvailable } from '@/lib/emailDeliveries';
...
export async function GET(request, { params }) {
  ...
  // Opens / clicks (Sam, 15 Sep 2026) need the email_opens migration as well.
  const draftOpenColumns = (await emailOpensAvailable(auth.adminSupabase)) ? ', opened_at, clicked_at' : '';
  ...
  const [{ data: batch, error: batchErr }, { data: leads, error: leadsErr }, { data: images, error: imagesErr }, { data: drafts, error: draftsErr }] =
    await Promise.all([
      ...
      auth.adminSupabase.from('fair_email_drafts').select(`id, lead_id, status, error, sent_at, message_id, language${draftDeliveryColumns}${draftOpenColumns}`).eq('batch_id', id),
    ]);
