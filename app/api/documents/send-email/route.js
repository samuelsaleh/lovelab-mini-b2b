...
// All client-facing order emails are BCC'd to the LoveLab office inboxes plus
// Alberto's personal Gmail so the team has an internal record without exposing
// those addresses to clients. Reply-to only includes the office inboxes (Dionne
// + Elie) so client replies funnel to shared inboxes. Hardcoded on purpose —
// admins can't accidentally bypass it.
const BCC_RECIPIENTS = ['dionne@love-lab.com', 'elie@love-lab.com', 'albertosaleh@gmail.com'];
...
const ADMIN_ALERT_EMAIL = process.env.ADMIN_ALERT_EMAIL || BCC_RECIPIENTS[0];
...
export async function POST(request) {
  try {
    ...
    // BCC is hardcoded to the LoveLab office inboxes — clients can't see these
    ...
    // Gmail Sent).
    const bccEmails = BCC_RECIPIENTS;
    ...
    // Reply-to is the same office inboxes as the internal BCC so all client replies
