 ...
 *
 * POST /api/anthropic — health-event wiring tests
 *
...
const { POST } = require('../anthropic/route');
...
function makeReq(body) {
  return {
    url: 'http://localhost/api/anthropic',
...
describe('POST /api/anthropic', () => {
  it('records a health event when upstream returns a non-OK response', async () => {
    global.fetch.mockResolvedValue({
      ok: false,
      status: 500,
      json: jest.fn().mockResolvedValue({ error: { message: 'overloaded' } }),
    });
  ...
  it('does NOT record a health event for a successful response', async () => {
    global.fetch.mockResolvedValue({
      ok: true,
      status: 200,
      json: jest.fn().mockResolvedValue({ content: [{ type: 'text', text: 'hello' }] }),
    });
