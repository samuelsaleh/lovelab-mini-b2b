...
/**
 * PATCH /api/igi/preview/alerts — set IGI's alert level from the preview.
 *
...
export async function PATCH(request) {
  const auth = await requireLoveLab(request, 'igi-preview-alerts', 30);
  ...
  try {
    const result = await setPoolMin(auth.adminSupabase, auth.user.id, body);
    if (result.error) return fail('IGI/Preview alerts', result.error, result.message);
    return NextResponse.json(result.body, { status: result.status });
  } catch (err) {
    return fail('IGI/Preview alerts', err, 'Internal server error');
  }
}
