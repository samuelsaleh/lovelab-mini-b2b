...
/**
 * GET /api/igi/preview/invoices — IGI's Invoices, served to a LoveLab admin.
 *
...
export async function GET(request) {
  ...
  try {
    return NextResponse.json(invoicesView(await loadIgiWorld(auth.adminSupabase)));
  } catch (err) {
    return fail('IGI/Preview invoices', err, 'Failed to load IGI’s screen');
  }
}
