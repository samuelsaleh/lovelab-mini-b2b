...
import {
  poolOf, shelfOf, askedRightNow, shelfStatus, orderStatus, unattributedTotal,
} from '@/lib/igi/derive';
...
export async function GET(request) {
  ...
  try {
    ...
    for (const r of [models, batches, lines, visits, snapshots, descriptions, counts, certIns, certOuts]) {
    ...
    const rows = models.data.map((m) => {
      ...
      return {
        ...
        // The one level on IGI's stock, set by LoveLab and seen by IGI.
