 ...
 *
 * Props:
 *   onOpenAccount — called when "My Account" is clicked (opens MyAccountPanel)
 */
export default function UserMenu({ onOpenAccount }) {
  ...
  // Close on outside click — use 'click' (not mousedown) to fix iOS sign-out.
  useEffect(() => {
    if (!open) return
    ...
    <div ref={menuRef} style={{ position: 'relative' }}>
      ...
      >
        {avatarUrl ? (
          <img
            src={avatarUrl}
            alt={displayName}
            style={{ width: mobile ? 34 : 28, height: mobile ? 34 : 28, borderRadius: '50%', objectFit: 'cover' }}
          />
        ) : (
          <div style={{
            width: mobile ? 34 : 28,
        ...
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ color: '#666' }}>
          <path d="M6 9l6 6 6-6"/>
        </svg>
      </button>
      ...
      {open && (
        <div style={{
          ...
          minWidth: 210, zIndex: 1000, overflow: 'hidden',
        }}>
          {/* User info header */}
          {user && (
          ...
          {/* My Account — opens MyAccountPanel */}
          {user && (
            <button
              data-testid="my-account-button"
              onPointerDown={(e) => { e.preventDefault(); setOpen(false); onOpenAccount?.() }}
              style={{
                ...menuItemStyle,
          ...
          {profile?.role === 'admin' && (
            <button
              ...
              style={{ ...menuItemStyle, color: colors.inkPlum, fontWeight: 600 }}
            ...
            <div style={{ borderBottom: '1px solid #eee' }}>
              {!showSetPassword ? (
                <button
                  ...
                  style={{ ...menuItemStyle, color: '#555', borderBottom: 'none' }}
                  onMouseEnter={(e) => e.currentTarget.style.background = '#f5f5f5'}
                  onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}
                >
          ...
          {/* Language Switcher */}
          <div style={{ padding: '8px 16px', borderBottom: '1px solid #eee', display: 'flex', gap: 4, alignItems: 'center' }}>
          ...
          {user ? (
            <button
              ...
              style={{
                ...
                border: 'none', background: mobile ? '#fef2f2' : 'transparent',
            ...
            <button
              ...
              style={{
                ...
                fontSize: 13, color: colors.inkPlum, cursor: 'pointer',
                fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 8,
                minHeight: mobile ? 48 : 'auto',
              }}
              onMouseEnter={(e) => e.currentTarget.style.background = '#f5f5f5'}
