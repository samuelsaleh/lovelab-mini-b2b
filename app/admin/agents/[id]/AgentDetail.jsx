...
/**
 * The admin view of one salesperson: orders, commissions, payments, reports.
 *
 * `kind` is 'agent' (Sales Team → Agents) or 'commercial' (Sales Team →
 * Commercials — an employee who takes orders and earns commission, Sam,
 * 15 Sep 2026). Same page, same numbers; only the breadcrumb and the words
 * differ, because a commercial is not an agent.
 */
export default function AgentDetail({ kind = 'agent' }) {
  const router = useRouter();
  const params = useParams();
  const agentId = params?.id;
  const isCommercial = kind === 'commercial';
  const listHref = isCommercial ? '/admin/commercials' : '/admin/agents';
  const noun = isCommercial ? 'commercial' : 'agent';
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!found) throw new Error(isCommercial ? 'Commercial not found' : 'Agent not found');
  ...
  const handleForcePaid = useCallback(async (commissionId) => {
    ...
    const ok = typeof window !== 'undefined'
      ? window.confirm(
        ...
        + `Use this when the ${noun} has already been paid but the line stayed on `
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px', fontFamily: fonts.body, background: '#f8f7fb' }}>
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        ...
        >
          Sales Team <span aria-hidden="true">/</span> {isCommercial ? 'Commercials' : 'Agents'} <span aria-hidden="true">/</span> <span style={{ color: colors.inkPlum }}>{agent?.full_name || (isCommercial ? 'Commercial' : 'Agent')}</span>
        </button>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isCompact ? '1fr' : 'repeat(3, 1fr)', gap: 12, marginBottom: 20 }}>
              {[
                ...
                },
                {
                  ...
                  sub: `transferred to ${noun}`,
                  accent: '#15803d',
                },
                {
                  label: 'OUTSTANDING',
                  value: fmt2(pendingBalance),
                  sub: `still owed to ${noun}`,
                  accent: '#b45309',
                },
              ].map(k => (
                    ...
                    return (
                      <>
                                      ...
                                      <div style={{ marginTop: 4 }}>
                                        <button
                                          ...
                                          title={isReported
                                            ...
                                            : `No report has been sent for this ${noun} yet.`}
                                    ...
                                    {canForcePaid && (
                                      <div style={{ marginTop: 4 }}>
                                        <button
                                          ...
                                          disabled={rowBusy}
                                          title={`Move this line to Paid by hand — for when the ${noun} was already paid but the line stayed on Reported. No payment is recorded, and Undo reverses it.`}
                        ...
                        <div style={{ padding: '10px 14px', borderTop: `1px solid ${colors.lineGray}`, fontSize: 11, color: colors.lovelabMuted, lineHeight: 1.5, background: '#fafafa' }}>
                          <strong style={{ color: colors.charcoal }}>Total</strong> = full invoice. <strong style={{ color: colors.charcoal }}>Net</strong> = Total − shipping. <strong style={{ color: colors.charcoal }}>Commission</strong> = Rate × Net. Tick <strong style={{ color: colors.charcoal }}>Paid?</strong> when the customer settles the order (→ <strong style={{ color: colors.charcoal }}>Ready</strong>). <strong style={{ color: colors.charcoal }}>Send report now</strong> emails Dionne (not the {noun}) and marks them <strong style={{ color: colors.charcoal }}>Reported</strong>. Then <strong style={{ color: colors.charcoal }}>Record Payment</strong> (pick the report + invoice) settles them as <strong style={{ color: colors.charcoal }}>Paid</strong>. If a line ended up on the wrong side of that, <strong style={{ color: colors.charcoal }}>Mark reported</strong> / <strong style={{ color: colors.charcoal }}>Not reported</strong> under the status moves it by hand, and <strong style={{ color: colors.charcoal }}>Force paid</strong> settles a line the {noun} was already paid for outside the app (<strong style={{ color: colors.charcoal }}>Undo</strong> reverses it).
                        </div>
                      </>
                    );
                  })()}
            ...
            {activeTab === 'consignment' && (
              <div style={{ background: '#fff', border: `1px solid ${colors.lineGray}`, borderRadius: 12, overflow: 'hidden' }}>
                ...
                {agentConsignmentOrders.length === 0 ? (
                  <div style={{ padding: 24, fontSize: 13, color: colors.lovelabMuted }}>No consignment orders assigned to this {noun}.</div>
                ) : (
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr style={{ background: '#faf8fc' }}>
