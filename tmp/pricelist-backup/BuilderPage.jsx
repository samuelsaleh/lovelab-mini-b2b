...
export function mkColorConfig(colorName, minC = 1) {
  return {
    ...
    cordType: null,
    thickness: null,
    // For collections with hasClosure (CUTY, CUBIX): 'braided' | 'nonBraided' | null
    closureType: null,
    qty: minC,
    priceOverride: null,
    certType: null,
...
// ─── Standard Packs (legacy) ───
// Pre-built order templates based on curated consignment selections. As of
// Phase 20 these live in the Supabase `packs` table (seeded by
// supabase-phase20-custom-packs.sql with `is_seed = true`). The constants
// below remain as a static reference / fallback for tests and scripts; the
// runtime BuilderPage no longer reads them — it fetches /api/packs.
...
export default function BuilderPage({ lines, setLines, onGenerateQuote, budget, setBudget, budgetRecommendations, showRecommendations, setShowRecommendations, onRequestRecommendations, orderChannel, pricelistYear, setPricelistYear }) {
  ...
  const { t } = useI18n()
  // Auth context for the custom-packs feature: we need the current user id
  // to render the "Mine" badge on agent-private packs and the admin role
  // to allow scope-toggling in the modal. Defensive about missing context
  // (some tests render BuilderPage without an AuthProvider).
  let _auth = null
  try { _auth = useAuth() } catch { _auth = null }
  const currentUser = _auth?.user || null
  const currentUserId = currentUser?.id || null
  const isAdmin = _auth?.profile?.role === 'admin'
  ...
  // ─── Custom packs (DB-backed) ───
  // Replaces the legacy hardcoded PACKS constant. The 4 legacy packs are
  // seeded into the table by the phase-20 migration so they appear here
  // identically; admins can publish more global packs and agents can save
  // private ones (visible only to themselves).
  ...
  const reloadPacks = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setPacks([])
        // Distinguish "table missing" (migration not yet run) from generic
        // server errors so admins know to apply the phase-20 migration
        // instead of staring at a silent empty list.
        const body = await res.json().catch(() => ({}))
        const msg = String(body?.error || '')
        if (res.status === 500 || /packs/i.test(msg)) {
          setPacksError('Packs unavailable. The database migration "supabase-phase20-custom-packs.sql" has not been applied yet — ask an admin to run it in the Supabase SQL editor.')
        } else {
          setPacksError(msg || `Could not load packs (HTTP ${res.status})`)
        }
        return
      }
      const data = await res.json().catch(() => ({ packs: [] }))
      // Normalize snake_case from the DB into the camelCase the rest of the
      // builder (and `applyPack`) already expects. This keeps the existing
      // rendering / apply paths byte-for-byte the same as the hardcoded
      // version, which is what the BuilderPage applyPack regression test
      // pins down.
      ...
      console.error('[BuilderPage] reloadPacks', err)
      setPacks([])
      setPacksError('Could not load packs. Check your connection and retry.')
    } finally {
  ...
  const deletePack = useCallback(async (packId) => {
    if (typeof window !== 'undefined' && !window.confirm(t('pack.deleteConfirm'))) return
    try {
      const res = await fetch(`/api/packs/${packId}`, { method: 'DELETE' })
      if (!res.ok) {
        const data = await res.json().catch(() => ({}))
        if (typeof window !== 'undefined') window.alert(data?.error || 'Failed to delete pack')
        return
      }
      setPacks(prev => prev.filter(p => p.id !== packId))
    } catch (err) {
      console.error('[BuilderPage] deletePack', err)
    }
  }, [t])
  ...
  return (
    <div style={{ display: 'flex', flex: 1, flexDirection: 'column', minHeight: 0, overflow: 'hidden', position: 'relative' }}>
          ...
          <div style={{ maxWidth: 900, margin: '0 auto' }}>
            ...
            <div style={{ marginBottom: 12 }}>
              <div style={{ display: 'flex', alignItems: 'stretch', gap: 8 }}>
                ...
                {/* "+ Create pack" — only shown when the user has built
                   something in the configure step. Saving a pack with zero
                   lines (or below €970) is blocked client-side AND
                   server-side, but we hide the button here for clarity. */}
                {currentUser && (
                  <button
                    type="button"
                    onClick={() => setShowPackBuilder(true)}
                    disabled={!hasContent}
                    title={hasContent ? t('pack.create') : t('pack.minPriceError')}
                    style={{
                      padding: '8px 12px', borderRadius: 10,
                      border: `1px solid ${hasContent ? colors.inkPlum : '#ddd'}`,
                      background: hasContent ? colors.inkPlum : '#f5f5f5',
              ...
              {showPacks && packsError && (
                <div role="alert" style={{ margin: '8px 2px 0', padding: '10px 14px', borderRadius: 10, background: '#fef2f2', border: '1px solid #fecaca', color: '#991b1b', fontSize: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
                  <span>{packsError}</span>
                  <button
                    type="button"
                    onClick={reloadPacks}
                    style={{ padding: '5px 12px', borderRadius: 6, border: '1px solid #b91c1c', background: '#fff', color: '#b91c1c', cursor: 'pointer', fontSize: 11, fontWeight: 700 }}
                  >
                    Retry
                  </button>
                </div>
              ...
              {showPacks && (
                <div style={{ display: 'flex', gap: 10, overflowX: 'auto', padding: '12px 2px 4px', scrollbarWidth: 'none' }}>
                  {packsLoading && packs.length === 0 && (
                    <div style={{ fontSize: 11, color: '#999', padding: 12 }}>Loading packs…</div>
                  )}
                  {!packsLoading && packs.length === 0 && !packsError && (
                    <div style={{ fontSize: 11, color: '#999', padding: 12, fontStyle: 'italic' }}>
                      No packs yet. {currentUser && t('pack.create')}
                    </div>
                  )}
                  {packs.map(pack => {
                    const isMine = !pack.isSeed && pack.scope === 'private' && pack.createdBy && pack.createdBy === currentUserId
                    const canDelete = !pack.isSeed && (
                      (pack.createdBy && pack.createdBy === currentUserId)
                      || (isAdmin && pack.scope === 'global')
                    )
                    return (
                    <div key={pack.id} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6, gap: 6 }}>
                        <div style={{ fontWeight: 700, fontSize: 13, color: colors.inkPlum, display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
                          <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{pack.label}</span>
                          {isMine && (
                            <span style={{
                              fontSize: 9, fontWeight: 700, padding: '2px 6px', borderRadius: 999,
                              background: '#efe1f4', color: colors.inkPlum, textTransform: 'uppercase', letterSpacing: 0.5,
                              flexShrink: 0,
                            }}>
                              {t('pack.badgeMine')}
                            </span>
                        ...
                        {canDelete && (
                          <button
                            ...
                            style={{
                              ...
                              color: '#e74c3c', fontSize: 10, lineHeight: '18px',
      ...
      {/* Save-as-pack modal — saving snapshots the current `lines` and posts
         to /api/packs. The modal handles the €970 minimum + scope rules. */}
      <PackBuilderModal
