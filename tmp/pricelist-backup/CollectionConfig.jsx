...
export default function CollectionConfig({ line, col, onChange, onRemove, selectedConfigs = new Set(), onToggleConfigSelect, onToggleLineSelect, recentlyDuplicated = new Set(), pricelistYear }) {
  ...
  const [duplicateSettings, setDuplicateSettings] = useState({
    ...
    closureType: { keepSame: true, value: null },
  ...
  const isConfigComplete = (cfg) => {
    ...
    // Bracelet thread closure: required for collections that opt-in (CUTY, CUBIX).
  ...
  const duplicateAllWithVariations = () => {
    ...
    const newConfigs = configsToDuplicate.map(cfg => {
      ...
      return {
        ...
        closureType: duplicateSettings.closureType.keepSame ? cfg.closureType : duplicateSettings.closureType.value,
  ...
  // "Non-braided" closure on the bracelet thread.
              ...
              {showDuplicatePanel && (
                <div style={{ padding: '12px', background: '#fff', borderTop: '1px solid #eee' }}>
                  ...
                  {[
                    ...
                    { field: 'closureType', label: t('quote.closure'), show: hasClosure },
                    ...
                    <div key={field} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: mobile ? 10 : 8, flexWrap: 'wrap' }}>
                        ...
                        {!duplicateSettings[field].keepSame && field === 'closureType' && (
                          <select
                            value={duplicateSettings.closureType.value || ''}
                            onChange={(e) => updateDuplicateSetting('closureType', { value: e.target.value || null })}
                            style={{ ...selectStyle, ...(mobile ? mobileSelectOverride : {}) }}
                          >
