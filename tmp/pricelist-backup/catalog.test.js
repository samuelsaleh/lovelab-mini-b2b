...
describe('calculateQuote', () => {
  ...
  // ─── Closure (CUTY/CUBIX) ──────────────────────────────────────────────
  // Bracelet thread closure ('braided' | 'nonBraided') should round-trip
  // for collections with hasClosure (CUTY, CUBIX) and stay null for everyone
  // else, so older drafts and non-closure collections never get spurious
  // closure data on the order form.

  it('propagates closureType for CUTY rows (hasClosure: true)', () => {
    const result = calculateQuote([
  ...
  it('propagates closureType for CUBIX rows (hasClosure: true)', () => {
    const result = calculateQuote([
  ...
  it('forces closureType to null for collections without hasClosure (M3)', () => {
    // Even if the upstream config carries a closureType (corrupted/legacy
    // payload), calculateQuote must strip it so the order form does not
    // render a closure value next to a HOLY/M3/etc. row.
    const result = calculateQuote([
  ...
  it('returns null closureType for CUTY when no closure is selected', () => {
    const result = calculateQuote([
  ...
  it('marks CUTY and CUBIX as hasClosure in the catalog', () => {
    const cuty = COLLECTIONS.find(c => c.id === 'CUTY')
  ...
  it('does not mark non-closure collections as hasClosure', () => {
    const nonClosure = ['M3', 'M4', 'M5', 'MF', 'SSF', 'SSPF', 'SSRG', 'SSRD', 'HOLY']
    for (const id of nonClosure) {
      const col = COLLECTIONS.find(c => c.id === id)
      expect(col?.hasClosure).toBeFalsy()
    }
  })
})
