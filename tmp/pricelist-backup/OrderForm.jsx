...
import { COLLECTIONS, HOUSING, CORD_COLORS, CORD_OPTIONS, CORD_TYPE_LABELS, CERT_LABELS, getPrice, getDefaultCert, resolvePricelist, DEFAULT_PRICELIST } from '@/lib/catalog'
...
function prefillRows(quote) {
  ...
  for (const ln of quote.lines) {
    ...
    rows.push({
      ...
      setting,
      bpColor: color,
      // Bracelet thread closure: only relevant for collections with hasClosure
      // (CUTY, CUBIX). Stored as the same 'braided' / 'nonBraided' values used
      // in the builder to keep round-tripping clean.
                    ...
                    return (
                      <tr key={globalIdx}>
                        {COLUMNS.map((col) => {
                          ...
                          // Bracelet thread closure: only meaningful for collections that opt-in (CUTY, CUBIX).
                          // Stored values match the builder: 'braided' | 'nonBraided'.
                          ...
                          const isNA = na && (
                            ...
                            // Closure column: hide for collections that don't opt-in
                            // (everything except CUTY and CUBIX).
